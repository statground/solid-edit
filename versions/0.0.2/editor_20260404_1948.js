      ...
      <div class="lre-app" id="lreApp">
        ...
        <dialog class="lre-dialog" id="lreMathDialog">
          <div class="lre-dialog-card">
            ...
            <div class="lre-dialog-body">
              <div class="lre-dialog-grid">
                <div class="lre-field"><span>Block math</span><div class="lre-note">Enter display TeX only. Inline math is removed.</div></div>
              </div>
...
/* 20260403-statkiss-selfboot-ui-math-fix-01 */
(function(){
  'use strict';
  if (typeof window === 'undefined') return;
  var PATCH_ID = '20260403-statkiss-selfboot-ui-math-fix-01';
  function log(){ try { console.info('[LocalRichEditor]', PATCH_ID, ...arguments); } catch (e) {} }
  function isFn(v){ return typeof v === 'function'; }
  function q(root, sel){ return root && root.querySelector ? root.querySelector(sel) : null; }
  function qa(root, sel){ return root && root.querySelectorAll ? Array.from(root.querySelectorAll(sel)) : []; }
  function esc(v){ return String(v == null ? '' : v).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;'); }
  function ensureStyle(){
    if (document.getElementById('statkiss-selfboot-style-01')) return;
    var style = document.createElement('style');
    style.id = 'statkiss-selfboot-style-01';
    style.textContent = [
      '.lre-root[data-toolbar-size="compact"]{--sk-btn:36px;--sk-gap:8px;--sk-radius:12px;}',
      '.lre-root[data-toolbar-size="normal"]{--sk-btn:40px;--sk-gap:9px;--sk-radius:13px;}',
      '.lre-root[data-toolbar-size="roomy"]{--sk-btn:44px;--sk-gap:10px;--sk-radius:14px;}',
      '.lre-root .lre-toolbar{gap:var(--sk-gap,8px)!important;align-items:flex-start!important;}',
      '.lre-root .lre-group{gap:6px!important;padding:6px!important;border-radius:14px!important;}',
      '.lre-root .lre-btn{width:var(--sk-btn,36px)!important;height:var(--sk-btn,36px)!important;min-width:var(--sk-btn,36px)!important;border-radius:var(--sk-radius,12px)!important;}',
      '.lre-root .lre-btn svg{width:20px!important;height:20px!important;}',
      '.lre-root .lre-icon-text{width:24px!important;height:22px!important;}',
      '.lre-root .statkiss-ui-select{min-width:118px;max-width:100%;height:var(--sk-btn,36px);padding:0 12px;border:1px solid var(--lre-border,#d7ddea);border-radius:12px;background:var(--lre-card,#fff);color:var(--lre-text,#0f172a);font-size:13px;font-weight:600;}',
      '.lre-root .statkiss-ui-select:focus{outline:none;border-color:var(--lre-primary,#2563eb);box-shadow:0 0 0 3px rgba(37,99,235,.12);}',
      '.lre-root .statkiss-toolbar-divider{width:1px;align-self:stretch;background:var(--lre-border,#d7ddea);margin:0 2px;}',
      '.lre-root .statkiss-toolbar-group-label{display:none;}',
      '.lre-root .statkiss-utility-group{margin-left:auto;}',
      '.lre-root .statkiss-toolbar-size-toggle.is-compact::after{content:"S";font-size:11px;font-weight:700;}',
      '.lre-root .statkiss-toolbar-size-toggle.is-normal::after{content:"M";font-size:11px;font-weight:700;}',
      '.lre-root .statkiss-toolbar-size-toggle.is-roomy::after{content:"L";font-size:11px;font-weight:700;}',
      '.lre-root .statkiss-toolbar-size-toggle svg{display:none!important;}',
      '.lre-root #lreInspectorPanel,.lre-root #lreOutlinePanel,.lre-root #lreSearchbar{background:linear-gradient(180deg,rgba(148,163,184,.08),rgba(148,163,184,.04));}',
      '.lre-root .statkiss-inspector-head{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:10px;}',
      '.lre-root .statkiss-inspector-actions{display:flex;align-items:center;gap:8px;}',
      '.lre-root .statkiss-inspector-close,.lre-root .statkiss-inspector-delete{display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border:1px solid var(--lre-border,#d7ddea);border-radius:12px;background:var(--lre-card,#fff);}',
      '.lre-root .statkiss-inspector-delete{color:#b91c1c;border-color:rgba(185,28,28,.18);}',
      '.lre-root .statkiss-math-wrap{display:flex;justify-content:center;align-items:center;min-height:48px;overflow:auto;}',
      '.lre-root .statkiss-math-image{display:block;max-width:100%;height:auto;margin:0 auto;}',
      '.lre-root .statkiss-math-placeholder{color:var(--lre-muted,#64748b);font-size:14px;}',
      '.lre-root .lre-math-node.is-error .statkiss-math-wrap{justify-content:flex-start;}',
      '.lre-root .lre-math-node pre{margin:0;white-space:pre-wrap;word-break:break-word;}',
      '.lre-root .lre-main-wrap.has-inspector #lreInspectorPanel[hidden]{display:none!important;}',
      '.lre-root .lre-main-wrap:not(.has-inspector) #lreInspectorPanel{display:none!important;}',
      '.lre-root .lre-main-wrap.has-outline #lreOutlinePanel,.lre-root .lre-main-wrap.has-inspector #lreInspectorPanel{background:linear-gradient(180deg,rgba(148,163,184,.08),rgba(148,163,184,.03));}',
      '.lre-root .lre-searchbar{background:linear-gradient(180deg,rgba(148,163,184,.08),rgba(148,163,184,.03));border:1px solid var(--lre-border,#d7ddea);border-radius:14px;}',
      '.lre-root .statkiss-hidden-action{display:none!important;}',
      '.lre-root .lre-editor-page{width:100%!important;max-width:none!important;min-width:0!important;}',
      '.lre-root .lre-surface{overflow:hidden;}',
      '.lre-root .lre-surface, .lre-root .lre-panel{min-width:0;}',
      '.lre-root .lre-main-wrap{align-items:start;}',
      '.lre-root .lre-main-wrap.has-inspector{grid-template-columns:minmax(0,1fr) 340px!important;}',
      '@media (max-width: 1080px){.lre-root .lre-main-wrap.has-inspector{grid-template-columns:minmax(0,1fr)!important;}.lre-root #lreInspectorPanel{order:2;}.lre-root .statkiss-utility-group{margin-left:0;}}',
      '@media (max-width: 760px){.lre-root{padding:8px!important;}.lre-root .lre-surface{padding:12px!important;}.lre-root .lre-editor-page{padding:18px 14px!important;min-height:52vh!important;}.lre-root .lre-toolbar{gap:7px!important;}.lre-root .lre-group{max-width:100%;}.lre-root .statkiss-utility-group{width:100%;margin-left:0!important;}.lre-root .statkiss-ui-select{min-width:104px;flex:1 1 112px;}}'
    ].join('');
    document.head.appendChild(style);
  }

  function svg(inner){ return '<svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.9">'+inner+'</svg>'; }
  function iconTrash(){ return svg('<path d="M6 7h12"/><path d="M9 7V5h6v2"/><path d="M8 7l1 12h6l1-12"/><path d="M10 11v5"/><path d="M14 11v5"/>'); }
  function iconClose(){ return svg('<path d="M7 7l10 10"/><path d="M17 7L7 17"/>'); }
  function iconSize(){ return svg('<path d="M7 17h10"/><path d="M7 12h10"/><path d="M9 7h6"/>'); }
  ...
  function createStatkissEditor(target, options){
    var host = toNode(target);
    if (!host) return Promise.reject(new Error('Editor host not found: ' + String(target)));
    var opts = Object.assign({}, options || {});
    if (host.tagName === 'TEXTAREA') return Promise.resolve(window.initStatkissContentEditor(host, opts));
    var proxy = host.querySelector('textarea[data-statkiss-editor-proxy="true"]');
    if (!proxy) {
      proxy = document.createElement('textarea');
      proxy.dataset.statkissEditorProxy = 'true';
      proxy.className = 'sk-ann-editor-textarea';
      proxy.setAttribute('rows', String(opts.rows || 18));
      proxy.style.display = 'none';
      host.appendChild(proxy);
    }
    if (typeof opts.html === 'string') proxy.value = opts.html;
    return Promise.resolve(window.initStatkissContentEditor(proxy, opts));
  }
  window.createStatkissEditor = createStatkissEditor;
  window.StatKISSEditor = window.StatKISSEditor || { mount: createStatkissEditor };

  function specialSelector(){ return '.lre-image-node,.lre-code-node,.lre-math-node,.latex-node,table'; }
  function isSpecialNode(node){ return !!(node && node.closest && node.closest(specialSelector())); }
  ...
  function findAlignBlock(editor, node){
    if (!editor || !editor.editor || !node || !node.closest) return editor && editor.editor || null;
    var block = node.closest('p,h1,h2,h3,blockquote,li,div');
    if (block && editor.editor.contains(block)) return block;
  ...
  function getTextAlignContext(editor){
    ...
    if (anchor) {
      if (isSpecialNode(anchor)) return { special:true, block:null };
      return { special:false, block: findAlignBlock(editor, anchor) || editor.editor };
    }
    if (editor.selectedNode && isSpecialNode(editor.selectedNode)) return { special:true, block:null };
    return { special:false, block: editor.editor };
  }
  function currentAlign(block){
    if (!block) return 'left';
  ...
  function ensureTextAlignButtons(editor){
    ...
    if (!group) {
      ...
      group.className = 'lre-group statkiss-text-align-group';
      [
        { align:'left', label:'Align left', svg: svg('<path d="M5 7h14"/><path d="M5 12h10"/><path d="M5 17h14"/>') },
        { align:'center', label:'Align center', svg: svg('<path d="M5 7h14"/><path d="M7 12h10"/><path d="M5 17h14"/>') },
        { align:'right', label:'Align right', svg: svg('<path d="M5 7h14"/><path d="M9 12h10"/><path d="M5 17h14"/>') }
      ].forEach(function(item){
        var btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'lre-btn';
        ...
        btn.title = item.label;
        btn.setAttribute('aria-label', item.label);
        btn.innerHTML = item.svg + '<span class="lre-sr">' + item.label + '</span>';
        btn.addEventListener('mousedown', function(ev){ ev.preventDefault(); });
        btn.addEventListener('click', function(ev){ ev.preventDefault(); applyTextAlign(editor, item.align); });
        group.appendChild(btn);
      });
  ...
  function normalizeTeX(value){
    var tex = String(value == null ? '' : value).replace(/\u00a0/g, ' ').trim();
    if (/^\$\$[\s\S]*\$\$$/.test(tex)) tex = tex.replace(/^\$\$|\$\$$/g, '').trim();
    if (/^\\\[[\s\S]*\\\]$/.test(tex)) tex = tex.replace(/^\\\[|\\\]$/g, '').trim();
    return tex;
  }
  function getMathNode(node){
  ...
  function waitForEmbeddedMathJax(timeoutMs){
    ...
    return new Promise(function(resolve){
      ...
      (function tick(){
        ...
        if (mj && typeof mj.tex2svgPromise === 'function') {
          try {
            var p = mj.startup && mj.startup.promise;
            if (p && typeof p.then === 'function') {
              p.then(function(){ resolve(mj); }).catch(function(){ resolve(mj); });
            } else {
              resolve(mj);
            }
            return;
          } catch (e) { resolve(mj); return; }
        }
  ...
  function extractMathSvg(result){
    if (!result) return '';
    var svg = null;
    if (result.tagName && String(result.tagName).toLowerCase() === 'svg') svg = result;
    if (!svg && result.querySelector) svg = result.querySelector('svg');
  ...
  async function renderMathNodePatched(editor, node, force){
    node = getMathNode(node);
    if (!node) return;
    node.classList.add('lre-math-node', 'block');
    node.classList.remove('inline', 'latex-node');
    ...
    if (!force && node.__statkissLastMathTex === tex && q(wrap, 'svg,img')) return;
    var token = (node.__statkissMathToken || 0) + 1;
    node.__statkissMathToken = token;
    wrap.innerHTML = '<span class="statkiss-math-placeholder">Rendering…</span>';
    try {
      var mj = await waitForEmbeddedMathJax(18000);
      if (token !== node.__statkissMathToken) return;
      if (!mj || typeof mj.tex2svgPromise !== 'function') throw new Error('MathJax not ready');
      var result = await mj.tex2svgPromise(tex, { display: true });
      if (token !== node.__statkissMathToken) return;
      var svgMarkup = extractMathSvg(result);
      if (!svgMarkup) throw new Error('MathJax produced no SVG');
    ...
    if (editor && isFn(editor.populateMathInspector) && isFn(editor.getSelectedMathNode) && editor.getSelectedMathNode() === node) editor.populateMathInspector(node);
  }
  function rerenderAllMath(editor, force){
    if (!editor || !editor.root) return;
    qa(editor.editor || editor.root, '.lre-math-node, .latex-node').forEach(function(node){ renderMathNodePatched(editor, node, force); });
    var preview = editor.sourcePreview || q(editor.root, '#lreMarkdownPreview');
    if (preview) qa(preview, '.lre-math-node, .latex-node').forEach(function(node){ renderMathNodePatched(editor, node, force); });
  }
  function placeCaretAtStart(el, editor){
    if (!el) return;
    var sel = window.getSelection && window.getSelection();
    ...
    try { editor && editor.saveSelection && editor.saveSelection(); } catch (e) {}
  }
  function ensureParagraphAfter(node){
    if (!node || !node.parentNode) return null;
    var next = node.nextElementSibling;
    if (next && /^(P|DIV)$/i.test(next.tagName) && !isSpecialNode(next)) return next;
  ...
  function finishBlockInsertion(editor, node){
    if (!editor || !node) return;
    try { editor.clearNodeSelection && editor.clearNodeSelection(); } catch (e) {}
    var p = ensureParagraphAfter(node);
    placeCaretAtStart(p, editor);
    try { editor.markDirty && editor.markDirty(); } catch (e) {}
    try { editor.updateOutline && editor.updateOutline(); } catch (e) {}
    try { editor.updateStatus && editor.updateStatus(); } catch (e) {}
    try { editor.updateSourceView && editor.updateSourceView(); } catch (e) {}
    try { editor.updateToolbarActiveState && editor.updateToolbarActiveState(); } catch (e) {}
  }

  function groupWrapper(cls){ var div = document.createElement('div'); div.className = 'lre-group ' + cls; return div; }
  function moveIf(group, node){ if (group && node) group.appendChild(node); }
  function buildSelect(className, title, options, onChange){
    var select = document.createElement('select');
    select.className = 'statkiss-ui-select ' + className;
    select.setAttribute('aria-label', title);
    select.title = title;
    options.forEach(function(opt){ var o = document.createElement('option'); o.value = opt.value; o.textContent = opt.label; select.appendChild(o); });
    select.addEventListener('mousedown', function(ev){ ev.stopPropagation(); });
    select.addEventListener('change', onChange);
    return select;
  }
  function syncHeadingSelect(editor){
    var select = q(editor && editor.root, '.statkiss-heading-select');
    if (!select) return;
    var anchor = getSelectionAnchor(editor);
    if (!anchor) { select.value = ''; return; }
    var block = anchor.closest ? anchor.closest('h1,h2,h3,p,blockquote,li') : null;
    if (!block || !editor.editor.contains(block)) { select.value = ''; return; }
    var tag = String(block.tagName || '').toLowerCase();
    select.value = tag === 'h1' ? 'h1' : tag === 'h2' ? 'h2' : tag === 'h3' ? 'h3' : tag === 'blockquote' ? 'quote' : 'paragraph';
  }
  function syncListSelect(editor){
    var select = q(editor && editor.root, '.statkiss-list-select');
    if (!select) return;
    var anchor = getSelectionAnchor(editor);
    if (!anchor) { select.value = ''; return; }
    var list = anchor.closest ? anchor.closest('ul,ol') : null;
    if (!list || !editor.editor.contains(list)) { select.value = ''; return; }
    if (String(list.getAttribute('data-checklist') || '').toLowerCase() === 'true') { select.value = 'check-list'; return; }
    select.value = String(list.tagName || '').toLowerCase() === 'ol' ? 'ordered-list' : 'bullet-list';
  }
  function ensureToolbarLayout(editor){
    if (!editor || !editor.toolbar) return;
    var toolbar = editor.toolbar;
    ensureTextAlignButtons(editor);
    var sizeGroup = q(toolbar, '.statkiss-toolbar-size-group');
    if (!sizeGroup) {
      sizeGroup = groupWrapper('statkiss-toolbar-size-group');
      var btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'lre-btn statkiss-toolbar-size-toggle is-compact';
      btn.innerHTML = iconSize() + '<span class="lre-sr">Toolbar size</span>';
      btn.addEventListener('mousedown', function(ev){ ev.preventDefault(); });
      btn.addEventListener('click', function(ev){ ev.preventDefault();
        var current = editor.root.dataset.toolbarSize || 'compact';
        var next = current === 'compact' ? 'normal' : current === 'normal' ? 'roomy' : 'compact';
        editor.root.dataset.toolbarSize = next;
        btn.classList.remove('is-compact','is-normal','is-roomy');
        btn.classList.add('is-' + next);
      });
      sizeGroup.appendChild(btn);
    }
    editor.root.dataset.toolbarSize = editor.root.dataset.toolbarSize || 'compact';

    qa(toolbar, '[data-action="inline-code"],[data-action="clear-format"],[data-action="comment"],[data-action="toggle-source"],[data-action="toggle-split"],[data-action="toggle-dark"],[data-action="inline-math"]').forEach(function(node){ node.classList.add('statkiss-hidden-action'); });

    var headingWrap = q(toolbar, '.statkiss-heading-wrap');
    if (!headingWrap) {
      headingWrap = groupWrapper('statkiss-heading-wrap');
      var headingSelect = buildSelect('statkiss-heading-select', 'Text style', [
        ...
        if (!value) return;
        if (value === 'quote') editor.handleAction('quote');
        else editor.handleAction(value);
        this.value = '';
        syncHeadingSelect(editor);
      });
      headingWrap.appendChild(headingSelect);
    }
    var listWrap = q(toolbar, '.statkiss-list-wrap');
    if (!listWrap) {
      listWrap = groupWrapper('statkiss-list-wrap');
      var listSelect = buildSelect('statkiss-list-select', 'List style', [
        { value:'', label:'List style' },
        ...
        var value = this.value;
        if (!value) return;
        editor.handleAction(value);
        this.value = '';
        syncListSelect(editor);
      });
      listWrap.appendChild(listSelect);
    }

    function btn(action){ return q(toolbar, '[data-action="' + action + '"]'); }
    var ribbon = q(toolbar, '[data-ribbon-toggle="true"]');
    var history = groupWrapper('statkiss-history-group');
    moveIf(history, ribbon);
    ['undo','redo'].forEach(function(a){ moveIf(history, btn(a)); });

    var text = groupWrapper('statkiss-text-group');
    ['bold','italic','underline','strike'].forEach(function(a){ moveIf(text, btn(a)); });
    moveIf(text, q(toolbar, '.statkiss-text-align-group'));
    ['link','text-color','bg-color'].forEach(function(a){ moveIf(text, btn(a)); });

    var structure = groupWrapper('statkiss-structure-group');
    moveIf(structure, headingWrap);
    moveIf(structure, listWrap);
    ['hr'].forEach(function(a){ moveIf(structure, btn(a)); });

    var special = groupWrapper('statkiss-special-group');
    ['table','image','code-block','block-math'].forEach(function(a){ moveIf(special, btn(a)); });

    var utility = groupWrapper('statkiss-utility-group');
    ['toggle-search','toggle-outline','toggle-inspector'].forEach(function(a){ moveIf(utility, btn(a)); });
    moveIf(utility, sizeGroup);

    qa(toolbar, '.statkiss-history-group,.statkiss-text-group,.statkiss-structure-group,.statkiss-special-group,.statkiss-utility-group').forEach(function(group){ if (group.parentNode === toolbar) group.remove(); });
    toolbar.innerHTML = '';
    [history, text, structure, special, utility].forEach(function(group){ if (group && group.children.length) toolbar.appendChild(group); });
  ...
  function ensureInspectorChrome(editor){
    var panel = editor && (editor.inspectorPanel || q(editor.root, '#lreInspectorPanel'));
    if (!panel) return;
    var title = q(panel, 'h2');
    if (!title) return;
    ...
    var actions = q(head, '.statkiss-inspector-actions');
    if (!actions) { actions = document.createElement('div'); actions.className = 'statkiss-inspector-actions'; head.appendChild(actions); }
    var del = q(actions, '.statkiss-inspector-delete');
    if (!del) {
      del = document.createElement('button'); del.type = 'button'; del.className = 'statkiss-inspector-delete';
      del.title = 'Delete selected node'; del.setAttribute('aria-label','Delete selected node'); actions.appendChild(del);
    }
    del.innerHTML = iconTrash() + '<span class="lre-sr">Delete selected node</span>';
    if (!del.__bound) {
      del.__bound = true;
      del.addEventListener('mousedown', function(ev){ ev.preventDefault(); });
      del.addEventListener('click', function(ev){ ev.preventDefault(); if (isFn(editor.deleteSelectedNode)) editor.deleteSelectedNode(); });
    ...
    var close = q(actions, '.statkiss-inspector-close');
    if (!close) {
      close = document.createElement('button'); close.type = 'button'; close.className = 'statkiss-inspector-close';
      close.title = 'Close inspector'; close.setAttribute('aria-label','Close inspector'); actions.appendChild(close);
    }
    close.innerHTML = iconClose() + '<span class="lre-sr">Close inspector</span>';
    if (!close.__bound) {
      close.__bound = true;
      close.addEventListener('mousedown', function(ev){ ev.preventDefault(); });
      close.addEventListener('click', function(ev){ ev.preventDefault();
        panel.hidden = true;
        if (editor.mainWrap) editor.mainWrap.classList.remove('has-inspector');
      });
    }
    // Keep delete to the left of close.
    if (actions.firstChild !== del) { actions.insertBefore(del, actions.firstChild); }
    if (del.nextSibling !== close) { actions.insertBefore(close, del.nextSibling); }
    qa(panel, '.lre-actions [data-action="delete-node"]').forEach(function(btn){ if (btn.parentNode) btn.parentNode.style.display = 'none'; });
  }

  function patchInstance(editor){
    if (!editor || editor.__STATKISS_SELFBOOT_PATCHED_01__) return editor;
    editor.__STATKISS_SELFBOOT_PATCHED_01__ = true;
    ensureStyle();
    ensureToolbarLayout(editor);
    ensureInspectorChrome(editor);

    var originalHandleAction = isFn(editor.handleAction) ? editor.handleAction.bind(editor) : null;
    editor.handleAction = function(action, button){
      if (action === 'inline-code' || action === 'clear-format' || action === 'toggle-source' || action === 'toggle-split' || action === 'toggle-dark' || action === 'comment' || action === 'inline-math') return;
      return originalHandleAction ? originalHandleAction(action, button) : undefined;
    };

    editor.renderMathNode = function(node, force){ return renderMathNodePatched(this, node, force); };
    editor.renderAllMath = function(force){ return rerenderAllMath(this, force); };
    ...
    editor.saveMathDialog = function(){
      ...
      node.dataset.tex = tex;
      node.dataset.display = 'true';
      renderMathNodePatched(this, node, true);
      try { this.mathDialog && this.mathDialog.close(); } catch (e) {}
      this.editingMathNode = null;
      if (creating) finishBlockInsertion(this, node);
      else {
    ...
    var oldUpdateToolbar = isFn(editor.updateToolbarActiveState) ? editor.updateToolbarActiveState.bind(editor) : null;
    editor.updateToolbarActiveState = function(){ if (oldUpdateToolbar) oldUpdateToolbar(); refreshUi(); };
    var oldSelectNode = isFn(editor.selectNode) ? editor.selectNode.bind(editor) : null;
    if (oldSelectNode) {
      editor.selectNode = function(node){ var r = oldSelectNode(node); var panel = editor.inspectorPanel || q(editor.root, '#lreInspectorPanel'); if (panel) { panel.hidden = false; if (editor.mainWrap) editor.mainWrap.classList.add('has-inspector'); } refreshUi(); return r; };
    }
    var oldClearNodeSelection = isFn(editor.clearNodeSelection) ? editor.clearNodeSelection.bind(editor) : null;
    if (oldClearNodeSelection) editor.clearNodeSelection = function(){ var r = oldClearNodeSelection(); refreshUi(); return r; };

    if (!editor.__skSelectionPatch){
      editor.__skSelectionPatch = true;
      document.addEventListener('selectionchange', function(){
        if (!editor.root || !editor.root.isConnected) return;
        var anchor = getSelectionAnchor(editor);
        if (anchor && !isSpecialNode(anchor) && editor.selectedNode && isSpecialNode(editor.selectedNode) && isFn(editor.clearNodeSelection)) editor.clearNodeSelection();
        refreshUi();
      });
      if (editor.editor) {
        editor.editor.addEventListener('mousedown', function(ev){
          var target = ev.target;
          if (target && !isSpecialNode(target) && editor.selectedNode && isSpecialNode(editor.selectedNode) && isFn(editor.clearNodeSelection)) {
            setTimeout(function(){ editor.clearNodeSelection(); refreshUi(); }, 0);
          }
        });
        editor.editor.addEventListener('click', refreshUi);
        editor.editor.addEventListener('keyup', refreshUi);
      }
    }

    // Remove display-mode selector for inline math and keep block-only mode.
    var mathDisplayWrap = q(editor.mathDialog || editor.root, '#lreMathDialogDisplay');
    if (mathDisplayWrap) {
      mathDisplayWrap.value = 'block';
      mathDisplayWrap.disabled = true;
      var field = mathDisplayWrap.closest('.lre-field');
      if (field) field.style.display = 'none';
    }
    var mathNote = q(editor.mathDialog || editor.root, '.lre-note');
    if (mathNote) mathNote.textContent = 'Formula renders inside the editor bundle. Block math only.';

    rerenderAllMath(editor, true);
    refreshUi();
    return editor;
  }
  ...
  function wrapMount(name){
    var fn = window[name];
    if (!isFn(fn) || fn.__STATKISS_SELFBOOT_WRAP_01__) return;
    var wrapped = function(){
      var result = fn.apply(this, arguments);
      if (result && isFn(result.then)) return result.then(function(editor){ return patchInstance(editor); });
      return patchInstance(result);
    };
    wrapped.__STATKISS_SELFBOOT_WRAP_01__ = true;
    window[name] = wrapped;
  }
...
/* 20260404-statkiss-clean-remove-unused-01 */
(function(){
  'use strict';
  if (typeof window === 'undefined') return;
  var PATCH_ID = '20260404-statkiss-clean-remove-unused-01';
  function log(){ try { console.info('[LocalRichEditor]', PATCH_ID, ...arguments); } catch (e) {} }
  function isFn(v){ return typeof v === 'function'; }
  function q(root, sel){ return root && root.querySelector ? root.querySelector(sel) : null; }
  function qa(root, sel){ return root && root.querySelectorAll ? Array.from(root.querySelectorAll(sel)) : []; }
  ...
  function cleanupEditor(editor){
    if (!editor || editor.__STATKISS_UNUSED_CLEANED_01__) return editor;
    editor.__STATKISS_UNUSED_CLEANED_01__ = true;

    // Remove unused DOM sections entirely.
    qa(editor.root, '[data-action="inline-code"],[data-action="clear-format"],[data-action="comment"],[data-action="inline-math"],[data-action="toggle-source"],[data-action="toggle-split"],[data-action="toggle-dark"]').forEach(removeNode);
    qa(editor.root, '.lre-comment-list').forEach(function(node){ var section = node.closest('.lre-inspector-section'); if (section) removeNode(section); });
    qa(editor.root, '#lreSourcePanel,.lre-source-panel').forEach(removeNode);
    qa(editor.root, '#lreMarkdownSource,#lreMarkdownPreview').forEach(function(node){ var card = node.closest('.lre-source-card'); if (card) removeNode(card); else removeNode(node); });
    qa(editor.root, '[data-action="comment"],[data-action="toggle-source"],[data-action="toggle-split"],[data-action="toggle-dark"],[data-action="inline-code"],[data-action="clear-format"],[data-action="inline-math"]').forEach(removeNode);

    // Neutralize obsolete methods.
    editor.comments = [];
    editor.addComment = function(){};
    editor.updateCommentList = function(){};
    editor.toggleSourcePanel = function(){ if (editor.state) editor.state.sourceOpen = false; };
    editor.toggleSplitView = function(){ if (editor.state) editor.state.split = false; };
    editor.toggleDarkMode = function(){};
    editor.wrapInlineCode = function(){};
    if (editor.mathDialogDisplay) {
      var wrap = editor.mathDialogDisplay.closest('.lre-field') || editor.mathDialogDisplay.parentNode;
      removeNode(wrap);
      editor.mathDialogDisplay = null;
    }
    if (isFn(editor.openMathDialog)) {
      var oldOpen = editor.openMathDialog.bind(editor);
      editor.openMathDialog = function(){ return oldOpen('block'); };
    }
    // ensure toolbar re-layout from previous patch if present
    if (isFn(editor.updateToolbarActiveState)) {
      try { editor.updateToolbarActiveState(); } catch (e) {}
    }
    return editor;
  }
  function wrapMount(name){
    var fn = window[name];
    if (!isFn(fn) || fn.__STATKISS_CLEAN_UNUSED_WRAP_01__) return;
    var wrapped = function(){
      var result = fn.apply(this, arguments);
      if (result && isFn(result.then)) return result.then(cleanupEditor);
      return cleanupEditor(result);
    };
    wrapped.__STATKISS_CLEAN_UNUSED_WRAP_01__ = true;
    window[name] = wrapped;
  }
  ['mountStatkissContentEditor','mountEmbeddedContentEditor','initStatkissContentEditor','initEmbeddedContentEditor','autoMountStatkissContentEditor'].forEach(wrapMount);
  if (window.localRichEditor) cleanupEditor(window.localRichEditor);
  log('applied');
})();
