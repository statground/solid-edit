...
/* SolidEdit 0.0.3: always-visible tools and explicit mini mode. */
(function(){
  'use strict';
  if (typeof window === 'undefined' || window.__SOLID_EDIT_UI_MODES_0_0_3__) return;
  window.__SOLID_EDIT_UI_MODES_0_0_3__ = true;
  window.SOLID_EDIT_VERSION = '0.0.3';

  var STYLE_ID = 'solid-edit-ui-modes-0-0-3';
  var WRAP_FLAG = '__solidEditUiModes003Wrapped';
  ...
  function ensureStyle(){
    ...
    style.id = STYLE_ID;
    style.textContent = [
      '.lre-root.lre-tools-always-visible .lre-ribbon-toggle-group{display:none!important;}',
      '.lre-root.lre-tools-always-visible .lre-group[data-ribbon-group="advanced"]:not([data-embed-empty="true"]){display:inline-flex!important;}',
      '.lre-root[data-solid-edit-mode="mini"]{--statkiss-toolbar-btn:32px;--statkiss-toolbar-icon:16px;--statkiss-toolbar-texticon-w:21px;--statkiss-toolbar-texticon-h:19px;}',
      '.lre-root[data-solid-edit-mode="mini"] .statkiss-toolbar-size-group{display:none!important;}',
      '.lre-root[data-solid-edit-mode="mini"] .lre-toolbar{gap:4px 3px!important;}',
      '.lre-root[data-solid-edit-mode="mini"] .lre-group{gap:3px!important;padding:4px!important;border-radius:11px!important;}',
      '.lre-root[data-solid-edit-mode="mini"] .lre-btn,.lre-root.lre-embedded-content[data-solid-edit-mode="mini"] .lre-btn{width:32px!important;height:32px!important;min-width:32px!important;min-height:32px!important;padding:6px!important;border-radius:10px!important;}',
      '.lre-root[data-solid-edit-mode="mini"] .lre-btn svg,.lre-root[data-solid-edit-mode="mini"] .tool-icon,.lre-root.lre-embedded-content[data-solid-edit-mode="mini"] .lre-btn svg{width:16px!important;height:16px!important;}',
      '.lre-root[data-solid-edit-mode="mini"] .lre-icon-text,.lre-root[data-solid-edit-mode="mini"] .tool-icon-text,.lre-root.lre-embedded-content[data-solid-edit-mode="mini"] .lre-icon-text{width:21px!important;height:19px!important;}',
      '.lre-root[data-solid-edit-mode="mini"] #lreEmbedToolbarHost{gap:6px!important;padding:6px!important;border-radius:14px!important;}',
      '@media (max-width:720px){.lre-root[data-solid-edit-mode="mini"] .lre-toolbar{gap:3px 2px!important;}.lre-root[data-solid-edit-mode="mini"] .lre-group{padding:3px!important;}}'
    ].join('');
    document.head.appendChild(style);
  }

  function normalizeMode(options, root){
    var opts = options && typeof options === 'object' ? options : {};
    if (Object.prototype.hasOwnProperty.call(opts, 'toolbarSize')) {
      return String(opts.toolbarSize || '').trim().toLowerCase() === 'mini' ? 'mini' : 'full';
    }
    return root && root.getAttribute('data-solid-edit-mode') === 'mini' ? 'mini' : 'full';
  }

  function removeRibbonToggle(root){
    if (!root || !root.querySelectorAll) return;
    Array.prototype.slice.call(root.querySelectorAll('[data-ribbon-toggle], .lre-ribbon-toggle-group')).forEach(function(node){
      var group = node.closest && node.closest('.lre-ribbon-toggle-group');
      var target = group || node;
  ...
  function applyMode(editor, options){
    ...
    ensureStyle();
    var mode = normalizeMode(options, editor.root);
    editor.root.classList.add('lre-tools-always-visible', 'lre-ribbon-expanded');
    editor.root.setAttribute('data-solid-edit-mode', mode);
    editor.root.dataset.toolbarSize = mode === 'mini' ? 'mini' : (editor.root.dataset.toolbarSize || 'compact');
    editor.state = editor.state || {};
    editor.state.ribbonExpanded = true;
    removeRibbonToggle(editor.root);
    if (typeof editor.updateLayout === 'function') editor.updateLayout();
    return editor;
  }

  function optionsForCall(name, args){
    if (/^(mountLocalRichEditor|createLocalRichEditor)$/.test(name)) return args[0] || {};
    if (/^(mountStatkissContentEditor|mountEmbeddedContentEditor|mountContentEditor)$/.test(name)) return args[1] || {};
    return args[0] || {};
  }
  ...
  function wrap(name){
    ...
    var wrapped = function(){
      var args = arguments;
      var options = optionsForCall(name, args);
      ensureStyle();
      var result = original.apply(this, args);
      if (result && isFn(result.then)) {
        return result.then(function(editor){ return applyMode(editor, options); });
      }
      return applyMode(result, options);
    };
    wrapped[WRAP_FLAG] = true;
    wrapped.__solidEditUiModes003Original = original;
    window[name] = wrapped;
  }
  ...
  window.addEventListener('local-rich-editor:ready', function(event){
    var config = window.CONTENT_EDITOR_CONFIG || window.STATKISS_SOLID_EDIT_CONFIG || {};
    applyMode(event && event.detail && event.detail.editor, config);
  });

  applyMode(window.__localRichEditor || window.localRichEditor, window.CONTENT_EDITOR_CONFIG || {});
})();
