/* Generic merged editor package 20260403: core + codeblock option + MathJax/highlight.js CDN support */
...
;(function bridgeGenericEditorHostGlobals() {
  if (typeof window === 'undefined') return;

  const PAIRS = [
    ['CONTENT_EDITOR_AUTOSTART', 'LOCAL_RICH_EDITOR_AUTOSTART'],
    ['CONTENT_EDITOR_AUTOINIT', 'STATKISS_SOLID_EDIT_AUTOINIT'],
    ['CONTENT_EDITOR_CONFIG', 'STATKISS_SOLID_EDIT_CONFIG'],
    ['CONTENT_EDITOR_TARGET', 'STATKISS_SOLID_EDIT_TARGET'],
    ['CONTENT_EDITOR_OPTIONS', 'STATKISS_SOLID_EDIT_OPTIONS'],
    ['CONTENT_EDITOR_BOOT_OPTIONS', 'STATKISS_SOLID_EDIT_BOOT_OPTIONS'],
    ['CONTENT_EDITOR_MATHJAX_CDN_URL', 'STATKISS_MATHJAX_CDN_URL'],
    ['CONTENT_EDITOR_MATHJAX_BUNDLE_URL', 'STATKISS_MATHJAX_BUNDLE_URL'],
    ['CONTENT_EDITOR_HLJS_SCRIPT_SRC', 'STATKISS_HLJS_SCRIPT_SRC'],
    ['CONTENT_EDITOR_HLJS_STYLE_HREF', 'STATKISS_HLJS_STYLE_HREF'],
    ['CONTENT_EDITOR_ENABLE_CODEBLOCK', 'STATKISS_ENABLE_CODEBLOCK'],
    ['ANNOUNCEMENT_CONTEXT', 'STATKISS_ANNOUNCEMENT_CONTEXT']
  ];
  ...
  PAIRS.forEach(function(pair) {
    var genericKey = pair[0];
    var legacyKey = pair[1];
    var initial = hasOwn(genericKey) ? window[genericKey] : (hasOwn(legacyKey) ? window[legacyKey] : undefined);
    var box = { value: initial };

    try { delete window[genericKey]; } catch (error) {}
    try { delete window[legacyKey]; } catch (error) {}

    function bind(key) {
      Object.defineProperty(window, key, {
        configurable: true,
        enumerable: true,
        get: function() { return box.value; },
        set: function(value) { box.value = value; }
      });
    }
    ...
    try {
      bind(genericKey);
      bind(legacyKey);
      if (initial !== undefined) window[genericKey] = initial;
    } catch (error) {
      if (!hasOwn(genericKey) && initial !== undefined) window[genericKey] = initial;
      if (!hasOwn(legacyKey) && initial !== undefined) window[legacyKey] = initial;
    }
  });
})();
;(function () {
  ...
  const STYLE = `
    ...
    * { box-sizing: border-box; }
    ...
    .lre-toolbar { display: flex; flex-wrap: wrap; align-items: center; gap: 4px 6px; min-width: 0; width: 100%; }
    .lre-group { display: inline-flex; flex-wrap: wrap; align-items: center; max-width: 100%; gap: 2px; padding: 0 6px 0 0; border: 0; border-inline-end: 1px solid var(--lre-border); border-radius: 0; background: transparent; }
    ...
    .lre-btn { position: relative; display: inline-flex; align-items: center; justify-content: center; width: 36px; height: 36px; border: 0; border-radius: var(--lre-btn-radius); background: transparent; color: var(--lre-text); cursor: pointer; transition: .12s ease; flex: 0 0 auto; }
    ...
    .lre-btn svg { width: 20px; height: 20px; }
    ...
    .lre-editor-page { min-height: 320px; width: min(100%, 980px); margin: 0 auto; padding: 24px; border: 0; border-radius: 0; background: var(--lre-card); outline: none; box-shadow: none; }
...
(function(){
  ...
  function setEnglishChrome(editor) {
    ...
    if (editor.imageInspector) {
      ...
      if (labels[0]) labels[0].childNodes[0] && (labels[0].childNodes[0].textContent = LABELS.caption + ' ');
      if (labels[1]) labels[1].childNodes[0] && (labels[1].childNodes[0].textContent = LABELS.alt + ' ');
      if (labels[2]) labels[2].childNodes[0] && (labels[2].childNodes[0].textContent = LABELS.width + ' ');
...
/* Embedded content editor mode for host pages such as write forms. */
(function(){
  'use strict';
  ...
  const EMBED_STYLE = `
    ...
    .lre-root.lre-embedded-content #lreEmbedToolbarHost {
      ...
      background: #f8fafc;
      border: 0;
      border-bottom: 1px solid var(--lre-border);
      border-radius: 0;
      box-shadow: none;
    }
      ...
      <div class="sk-demo-page">
        <header class="sk-demo-header">
          <div class="sk-demo-brand">
            <div class="sk-demo-logo">ED</div>
            <div>
              <strong>Reusable Content Editor</strong>
  ...
  window.mountContentEditor = function () {
    return window.mountStatkissContentEditor.apply(this, arguments);
  };
  window.autoMountContentEditor = function () {
    return window.autoMountStatkissContentEditor.apply(this, arguments);
  };
  window.initContentEditor = function () {
    return window.initStatkissContentEditor.apply(this, arguments);
  };
  window.renderContentWriteDemo = function () {
    return window.renderStatkissContentWriteDemo.apply(this, arguments);
  };
...
(function(){
  if (typeof window === 'undefined') return;
  window.STATKISS_MATHJAX_CDN_URL = window.STATKISS_MATHJAX_CDN_URL || 'https://cdn.jsdelivr.net/gh/mathjax/MathJax@3.2.2/es5/tex-svg.js';
  window.__STATKISS_ULTRA_PATCH__ = 'statkiss-optimizer-math-center-20260403-01';
...
/* StatKISS 20260403 announcement writer stabilization patch */
(function(){
  if (typeof window === 'undefined') return;
  if (window.__STATKISS_ANNOUNCEMENT_WRITER_STABLE_PATCH__) return;
  window.__STATKISS_ANNOUNCEMENT_WRITER_STABLE_PATCH__ = true;

  var MATHJAX_URL = window.STATKISS_MATHJAX_CDN_URL || 'https://cdn.jsdelivr.net/gh/mathjax/MathJax@3.2.2/es5/tex-svg.js';
  var STYLE_ID = 'statkiss-announcement-writer-stable-style';

  function isFn(fn){ return typeof fn === 'function'; }
  function q(root, sel){ try { return root ? root.querySelector(sel) : null; } catch (e) { return null; } }
  function qa(root, sel){ try { return Array.prototype.slice.call(root ? root.querySelectorAll(sel) : []); } catch (e) { return []; } }
  function esc(value){ return String(value == null ? '' : value).replace(/[&<>"']/g, function(ch){ return ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'})[ch]; }); }
  function log(){ try { var args = Array.prototype.slice.call(arguments); args.unshift('[LocalRichEditor]'); console.info.apply(console, args); } catch (e) {} }
  function sameUrl(a, b){ try { return new URL(a, document.baseURI).href === new URL(b, document.baseURI).href; } catch (e) { return a === b; } }
  ...
  function ensureStyle(){
    ...
    style.id = STYLE_ID;
    style.textContent = [
      '.lre-root .statkiss-math-wrap{display:flex;align-items:center;justify-content:center;min-height:1.8em;max-width:100%;width:100%;}',
      '.lre-root .lre-math-node.inline .statkiss-math-wrap{display:inline-flex;width:auto;min-width:1em;}',
      '.lre-root .statkiss-math-image{display:block;max-width:100%;height:auto;margin:0 auto;}',
      '.lre-root .lre-math-node.is-error .statkiss-math-wrap{justify-content:flex-start;text-align:left;}',
      '.lre-root .lre-math-node.is-error .statkiss-math-wrap pre{margin:0;white-space:pre-wrap;word-break:break-word;font:500 13px/1.55 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;color:#b91c1c;}',
      '.lre-root .lre-code-node .statkiss-code-toggle svg{width:18px;height:18px;display:block;}'
    ].join('');
    document.head.appendChild(style);
  }
  ...
  function awaitMathStartup(mj){
    return new Promise(function(resolve){
      try {
        var promise = mj && mj.startup && mj.startup.promise;
        if (promise && isFn(promise.then)) {
          var settled = false;
          function finish(){ if (!settled) { settled = true; resolve(mj); } }
  ...
  function ensureMathJax(){
    if (hasMathRuntime()) return awaitMathStartup(window.MathJax);
    if (window.__STATKISS_ANNOUNCEMENT_MATHJAX_PROMISE__) return window.__STATKISS_ANNOUNCEMENT_MATHJAX_PROMISE__;
    ensureMathConfig();
    window.__STATKISS_ANNOUNCEMENT_MATHJAX_PROMISE__ = new Promise(function(resolve, reject){
      if (hasMathRuntime()) { awaitMathStartup(window.MathJax).then(resolve); return; }
      var existing = null;
      qa(document, 'script[src]').some(function(node){
        var src = node.getAttribute('src') || node.src || '';
        if (src && sameUrl(src, MATHJAX_URL)) { existing = node; return true; }
        return false;
      });
      function finish(){
        if (!hasMathRuntime()) return false;
        awaitMathStartup(window.MathJax).then(resolve);
        return true;
      }
      if (finish()) return;
      var script = existing || document.createElement('script');
      if (!existing) {
        ...
        script.async = true;
        script.defer = true;
        script.dataset.statkissAnnouncementMathjax = '1';
        document.head.appendChild(script);
      }
      var settled = false;
      function ok(){ if (settled) return; if (finish()) settled = true; }
      function fail(){ if (settled) return; settled = true; reject(new Error('Failed to load MathJax runtime.')); }
      script.addEventListener('load', ok, { once: true });
      script.addEventListener('error', fail, { once: true });
      setTimeout(function(){ if (!settled) { if (finish()) { settled = true; return; } fail(); } }, 22000);
    });
    return window.__STATKISS_ANNOUNCEMENT_MATHJAX_PROMISE__;
  }
  ...
  function normalizeMathNode(node){
    ...
    node.dataset.type = 'math';
    if (!node.dataset.tex && node.dataset.latex) node.dataset.tex = node.dataset.latex;
    if (!node.dataset.display) node.dataset.display = node.classList.contains('block') ? 'true' : 'false';
    node.classList.toggle('block', String(node.dataset.display) === 'true');
    node.classList.toggle('inline', String(node.dataset.display) !== 'true');
    node.setAttribute('contenteditable', 'false');
    return node;
  }

  function ensureMathWrap(node){
    var wrap = q(node, '.statkiss-math-wrap');
    if (!wrap) {
      node.innerHTML = '';
  ...
  async function renderMathNode(editor, node, force){
    ...
    if (!target) return;
    var tex = String(target.dataset.tex || '').trim();
    var display = String(target.dataset.display || 'false') === 'true';
    target.classList.toggle('block', display);
    target.classList.toggle('inline', !display);
    var wrap = ensureMathWrap(target);
    var token = (target.__statkissRenderToken || 0) + 1;
    ...
    if (!force && target.dataset.renderedTex === tex && q(wrap, 'img.statkiss-math-image')) return;
    wrap.textContent = 'Rendering...';
    try {
      var mj = await ensureMathJax();
      if (target.__statkissRenderToken !== token) return;
      if (!mj || !(isFn(mj.tex2svgPromise) || isFn(mj.tex2svg))) throw new Error('MathJax runtime unavailable');
      var result = isFn(mj.tex2svgPromise) ? await mj.tex2svgPromise(tex, { display: display }) : mj.tex2svg(tex, { display: display });
      if (target.__statkissRenderToken !== token) return;
      var svgMarkup = extractSvgMarkup(result);
      if (!svgMarkup) throw new Error('MathJax SVG output missing');
      var image = document.createElement('img');
      image.className = 'statkiss-math-image';
      image.alt = tex;
  ...
  function renderAllMath(editor, force){
    if (!editor || !editor.root) return;
    qa(editor.editor || editor.root, '.lre-math-node, .latex-node').forEach(function(node){ renderMathNode(editor, node, force); });
    var preview = editor.sourcePreview || q(editor.root, '#lreMarkdownPreview') || q(editor.root, '#markdownPreview');
    if (preview) qa(preview, '.lre-math-node, .latex-node').forEach(function(node){ renderMathNode(editor, node, force); });
  }
  ...
  function fixImageInspectorLabels(editor){
    var root = editor && editor.root;
    if (!root) return;
    var inspector = editor.imageInspector || q(root, '#lreImageInspector') || q(root, '#imageInspector');
    if (!inspector) return;
    var captionInput = q(inspector, '#lreImageCaption') || q(inspector, '#imageCaptionInput');
    var altInput = q(inspector, '#lreImageAlt') || q(inspector, '#imageAltInput');
    var widthRange = q(inspector, '#lreImageWidth') || q(inspector, '#imageWidthRange');
    var alignSelect = q(inspector, '#imageAlignSelect');
    if (captionInput) {
      var captionLabel = captionInput.closest('label');
      if (captionLabel) setLeadingLabelText(captionLabel, 'Caption ');
      captionInput.setAttribute('placeholder', 'Caption');
    }
    if (altInput) {
      var altLabel = altInput.closest('label');
      if (altLabel) setLeadingLabelText(altLabel, 'Alt text ');
      altInput.setAttribute('placeholder', 'Alt text');
    }
    if (widthRange) {
      var widthField = widthRange.closest('.lre-field, .inspector-grid, label, div');
      var widthSpan = widthField && q(widthField, 'span');
      if (widthSpan) widthSpan.textContent = 'Width';
      if (!widthSpan) {
        var prev = widthRange.parentElement && widthRange.parentElement.previousElementSibling;
        if (prev) prev.textContent = 'Width';
      }
    }
    if (alignSelect) {
      var alignField = alignSelect.closest('.lre-field, .inspector-grid, label, div');
      var alignSpan = alignField && q(alignField, 'span');
      if (alignSpan) alignSpan.textContent = 'Align';
    }
  }

  function patchInstance(editor){
    if (!editor || editor.__STATKISS_ANNOUNCEMENT_WRITER_PATCHED__) return editor;
    editor.__STATKISS_ANNOUNCEMENT_WRITER_PATCHED__ = true;
    ensureStyle();
    fixImageInspectorLabels(editor);
    ...
    editor.updateInspector = function(){
      if (originalUpdateInspector) originalUpdateInspector();
      fixImageInspectorLabels(this);
    };

    var originalUpdateMarkdownPreview = isFn(editor.updateMarkdownPreview) ? editor.updateMarkdownPreview.bind(editor) : null;
    editor.updateMarkdownPreview = function(){
      if (originalUpdateMarkdownPreview) originalUpdateMarkdownPreview();
      renderAllMath(this, true);
    };

    var originalNormalizeAfterImport = isFn(editor.normalizeAfterImport) ? editor.normalizeAfterImport.bind(editor) : null;
    editor.normalizeAfterImport = function(){
      if (originalNormalizeAfterImport) originalNormalizeAfterImport();
      fixImageInspectorLabels(this);
      renderAllMath(this, true);
    };

    var originalSaveMathDialog = isFn(editor.saveMathDialog) ? editor.saveMathDialog.bind(editor) : null;
    editor.saveMathDialog = function(){
      if (originalSaveMathDialog) originalSaveMathDialog();
      fixImageInspectorLabels(this);
      renderAllMath(this, true);
    };

    var originalSetHTML = isFn(editor.setHTML) ? editor.setHTML.bind(editor) : null;
    if (originalSetHTML) {
      editor.setHTML = function(html){ var r = originalSetHTML(html); renderAllMath(this, true); fixImageInspectorLabels(this); return r; };
    }
    var originalSetMarkdown = isFn(editor.setMarkdown) ? editor.setMarkdown.bind(editor) : null;
    if (originalSetMarkdown) {
      editor.setMarkdown = function(markdown){ var r = originalSetMarkdown(markdown); renderAllMath(this, true); fixImageInspectorLabels(this); return r; };
    }
  ...
  function wrapMount(name){
    var fn = window[name];
    if (!isFn(fn) || fn.__STATKISS_ANNOUNCEMENT_WRITER_WRAP__) return;
    var wrapped = function(){
      var result = fn.apply(this, arguments);
      if (result && isFn(result.then)) return result.then(function(editor){ return patchInstance(editor); });
      return patchInstance(result);
    };
    wrapped.__STATKISS_ANNOUNCEMENT_WRITER_WRAP__ = true;
    window[name] = wrapped;
  }

  ['mountStatkissContentEditor','mountEmbeddedContentEditor','initStatkissContentEditor','initEmbeddedContentEditor','autoMountStatkissContentEditor'].forEach(wrapMount);
  if (window.localRichEditor) patchInstance(window.localRichEditor);
  log('statkiss-announcement-writer-stable-20260403-01 applied');
})();
/* StatKISS 20260403 announcement refinement patch: stable inspector + numbered captions */
(function(){
  'use strict';
  if (typeof window === 'undefined') return;
  if (window.__STATKISS_ANNOUNCEMENT_REFINEMENT_PATCH_20260403__) return;
  window.__STATKISS_ANNOUNCEMENT_REFINEMENT_PATCH_20260403__ = true;

  var STYLE_ID = 'statkiss-announcement-refinement-style-20260403';

  function isFn(fn){ return typeof fn === 'function'; }
  function q(root, sel){ try { return root ? root.querySelector(sel) : null; } catch (e) { return null; } }
  function qa(root, sel){ try { return Array.prototype.slice.call(root ? root.querySelectorAll(sel) : []); } catch (e) { return []; } }
  function escapeRegex(value){ return String(value == null ? '' : value).replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); }
  function log(msg){ try { console.info('[LocalRichEditor]', msg); } catch (e) {} }

  function svg(paths, viewBox){
    return '<svg class="lre-icon" viewBox="' + (viewBox || '0 0 24 24') + '" aria-hidden="true" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.9">' + paths + '</svg>';
  }
  function xIcon(){ return svg('<path d="M6 6 18 18"/><path d="M18 6 6 18"/>'); }
  function trashIcon(){ return svg('<path d="M3 6h18"/><path d="M8 6V4h8v2"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/>'); }
  ...
  function ensureStyle(){
    ...
    style.id = STYLE_ID;
    style.textContent = [
      '.statkiss-inspector-head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:8px;}',
      '.statkiss-inspector-head h2{margin:0;}',
      '.statkiss-inspector-actions{display:flex;align-items:center;gap:8px;margin-left:auto;}',
      '.statkiss-inspector-delete,.statkiss-inspector-close{display:inline-flex;align-items:center;justify-content:center;width:40px;height:40px;border:1px solid var(--lre-border,#d7ddea);border-radius:14px;background:transparent;color:var(--lre-text,#0f172a);cursor:pointer;transition:.18s ease;}',
      '.statkiss-inspector-delete:hover,.statkiss-inspector-close:hover{border-color:var(--lre-primary,#2563eb);background:color-mix(in srgb, var(--lre-primary-soft,#dbeafe) 70%, transparent);color:var(--lre-primary,#2563eb);}',
      '.statkiss-inspector-delete{color:#c2410c;border-color:rgba(194,65,12,.22);}',
      '.statkiss-inspector-delete:hover{color:#dc2626;border-color:#dc2626;background:rgba(220,38,38,.08);}',
      '.statkiss-inspector-delete svg,.statkiss-inspector-close svg{width:18px;height:18px;display:block;}',
      '.statkiss-node-caption{display:block;margin-top:10px;font-size:13px;line-height:1.5;color:var(--lre-muted,#64748b);text-align:center;}',
      'table > caption.statkiss-node-caption{display:table-caption;text-align:left;padding-bottom:8px;margin-top:0;}',
      '.lre-code-node > .statkiss-node-caption{padding:0 18px 16px;margin-top:0;}',
      '.lre-math-node > .statkiss-node-caption{padding-top:10px;}',
      '.statkiss-caption-field{display:grid;gap:6px;}',
      '.statkiss-caption-field input{width:100%;}',
      '.statkiss-inspector-manual-hidden .show-inspector{display:none !important;}'
    ].join('');
    document.head.appendChild(style);
  }
  ...
  function getNodeKind(node){
    if (!node) return '';
    if (node.classList && node.classList.contains('lre-image-node')) return 'image';
    if (node.tagName === 'TABLE') return 'table';
    if ((node.classList && node.classList.contains('lre-math-node')) || (node.classList && node.classList.contains('latex-node'))) return 'math';
    if (node.classList && node.classList.contains('lre-code-node')) return 'code';
    return '';
  }

  function getInspectableNode(target){
    if (!target || !target.closest) return null;
    var node = target.closest('.lre-image-node, .lre-code-node, .lre-math-node, .latex-node, table');
    return getNodeKind(node) === 'math' ? normalizeMathNode(node) : node;
  }

  function getSelectedNode(editor){
    if (!editor) return null;
    if (isFn(editor.getSelectedImageNode) && editor.getSelectedImageNode()) return editor.getSelectedImageNode();
    if (isFn(editor.getSelectedTable) && editor.getSelectedTable()) return editor.getSelectedTable();
    if (isFn(editor.getSelectedMathNode) && editor.getSelectedMathNode()) return normalizeMathNode(editor.getSelectedMathNode());
    if (isFn(editor.getSelectedCodeNode) && editor.getSelectedCodeNode()) return editor.getSelectedCodeNode();
    var direct = editor.selectedNode;
    return getNodeKind(direct) === 'math' ? normalizeMathNode(direct) : direct;
  }

  function getInspectorPanel(editor){
    return editor && (editor.inspectorPanel || q(editor.root, '#lreInspectorPanel') || q(editor.root, '#inspectorPanel') || q(editor.root, '.panel.right'));
  }

  function hideInspector(editor, manual){
    var panel = getInspectorPanel(editor);
    if (!panel) return;
    if (manual) editor.__statkissManualInspectorHidden = true;
    panel.hidden = true;
    if (editor.mainWrap) editor.mainWrap.classList.remove('has-inspector');
    if (editor.root) editor.root.classList.remove('show-inspector');
  }

  function showInspector(editor){
    var panel = getInspectorPanel(editor);
    if (!panel) return;
    editor.__statkissManualInspectorHidden = false;
    panel.hidden = false;
    if (editor.mainWrap) editor.mainWrap.classList.add('has-inspector');
    if (editor.root) editor.root.classList.add('show-inspector');
  }

  function ensureStatePointers(editor, node){
    if (!editor || !editor.state) return;
    editor.state.selectedImageFigure = getNodeKind(node) === 'image' ? node : null;
    editor.state.selectedTable = getNodeKind(node) === 'table' ? node : null;
    editor.state.selectedMathNode = getNodeKind(node) === 'math' ? node : null;
    editor.state.selectedLatex = editor.state.selectedMathNode;
    editor.state.selectedCodeFigure = getNodeKind(node) === 'code' ? node : null;
    editor.state.selectedCodeBlock = editor.state.selectedCodeFigure;
  }

  function cleanupInspectorHeader(editor){
    var panel = getInspectorPanel(editor);
    if (!panel) return;
    var title = q(panel, 'h2');
    if (!title) return;
    ...
    if (!head) {
      ...
      title.parentNode.insertBefore(head, title);
      head.appendChild(title);
    } else if (title.parentNode !== head) {
      head.insertBefore(title, head.firstChild || null);
    }
    ...
    del.title = 'Delete selected node';
    del.setAttribute('aria-label', 'Delete selected node');
    del.innerHTML = trashIcon() + '<span class="lre-sr">Delete selected node</span>';
    del.addEventListener('mousedown', function(ev){ ev.preventDefault(); });
    del.addEventListener('click', function(ev){ ev.preventDefault(); if (isFn(editor.deleteSelectedNode)) editor.deleteSelectedNode(); });
    actions.appendChild(del);
    ...
    close.title = 'Close inspector';
    close.setAttribute('aria-label', 'Close inspector');
    close.innerHTML = xIcon() + '<span class="lre-sr">Close inspector</span>';
    close.addEventListener('mousedown', function(ev){ ev.preventDefault(); });
    close.addEventListener('click', function(ev){ ev.preventDefault(); hideInspector(editor, true); });
    actions.appendChild(close);

    qa(panel, '.lre-actions [data-action="delete-node"]').forEach(function(btn){ if (btn.parentNode) btn.parentNode.style.display = 'none'; });
    qa(panel, '.statkiss-inspector-head > .statkiss-inspector-close').forEach(function(btn){
      if (btn.parentNode === head && btn.parentNode !== actions) btn.remove();
    });
  }

  function captionLabel(kind){
    return ({ image:'Image Caption', table:'Table Caption', math:'Formula Caption', code:'Code Caption' })[kind] || 'Caption';
  }

  function captionPrefix(kind, index){
    return captionLabel(kind) + ' ' + index;
  }

  function stripGeneratedPrefix(kind, value){
    var text = String(value == null ? '' : value).trim();
    if (!text) return '';
    var re = new RegExp('^' + escapeRegex(captionLabel(kind)) + '\\s+\\d+\\s*(?:[:.\\-–—]\\s*)?', 'i');
    return text.replace(re, '').trim();
  }

  function findDirectCaption(node, kind){
    if (!node) return null;
    if (kind === 'image') return q(node, 'figcaption');
    if (kind === 'table') return q(node, 'caption');
    var children = Array.prototype.slice.call(node.children || []);
    for (var i = 0; i < children.length; i += 1) {
      if (children[i].classList && children[i].classList.contains('statkiss-node-caption')) return children[i];
    }
    return null;
  }

  function ensureCaptionElement(node, kind){
    var el = findDirectCaption(node, kind);
    if (el) {
      el.classList.add('statkiss-node-caption');
      return el;
    }
    if (kind === 'image') {
      el = document.createElement('figcaption');
      el.className = 'statkiss-node-caption';
      node.appendChild(el);
      return el;
    }
    if (kind === 'table') {
      ...
      el.className = 'statkiss-node-caption';
      node.insertBefore(el, node.firstChild || null);
      return el;
    }
    if (kind === 'math') {
  ...
  function removeCaptionElement(node, kind){
    var el = findDirectCaption(node, kind);
    if (el) el.remove();
  }

  function isCaptionEnabled(node, kind){
    if (!node) return false;
    var stored = node.getAttribute('data-statkiss-caption-enabled');
  ...
  function getCaptionBase(node, kind){
    if (!node) return '';
    var stored = node.getAttribute('data-statkiss-caption-base');
    if (stored != null) return String(stored);
    var el = findDirectCaption(node, kind);
    var raw = el ? String(el.textContent || '').trim() : '';
    var base = raw ? stripGeneratedPrefix(kind, raw) : '';
    if (raw) node.setAttribute('data-statkiss-caption-enabled', '1');
    node.setAttribute('data-statkiss-caption-base', base);
    return base;
  }

  function setCaptionState(node, enabled, base){
    if (!node) return;
    node.setAttribute('data-statkiss-caption-enabled', enabled ? '1' : '0');
    node.setAttribute('data-statkiss-caption-base', String(base == null ? '' : base).trim());
  }

  function applyCaption(node, kind, index){
    if (!node) return false;
    var base = String(getCaptionBase(node, kind) || '').trim();
    var enabled = isCaptionEnabled(node, kind) || !!base;
    if (!enabled && !base) {
      removeCaptionElement(node, kind);
      node.removeAttribute('data-statkiss-caption-index');
      if (kind === 'image') {
        var img0 = q(node, 'img');
        if (img0) img0.alt = '';
      }
    ...
    var el = ensureCaptionElement(node, kind);
    var prefix = captionPrefix(kind, index);
    var text = base ? (prefix + ': ' + base) : prefix;
    if (el) el.textContent = text;
    node.setAttribute('data-statkiss-caption-enabled', '1');
    node.setAttribute('data-statkiss-caption-index', String(index));
    if (kind === 'image') {
      var img = q(node, 'img');
      if (img) img.alt = base || text;
    }
    return true;
  }

  function refreshCaptions(editor){
    if (!editor || !editor.editor || editor.__statkissCaptionRefreshing) return;
    editor.__statkissCaptionRefreshing = true;
    try {
      var counts = { image: 0, table: 0, math: 0, code: 0 };
      qa(editor.editor, '.lre-image-node, table, .lre-math-node, .latex-node, .lre-code-node').forEach(function(node){
        var kind = getNodeKind(node);
        if (!kind) return;
        if (kind === 'math') node = normalizeMathNode(node);
        if (applyCaption(node, kind, counts[kind] + 1)) counts[kind] += 1;
      });
      syncCaptionInputs(editor);
    } finally {
      editor.__statkissCaptionRefreshing = false;
    }
  }

  function scheduleCaptionRefresh(editor){
    if (!editor) return;
    clearTimeout(editor.__statkissCaptionRefreshTimer);
    editor.__statkissCaptionRefreshTimer = setTimeout(function(){ refreshCaptions(editor); }, 30);
  }

  function ensureCaptionObserver(editor){
    if (!editor || !editor.editor || editor.__statkissCaptionObserver) return;
    var observer = new MutationObserver(function(mutations){
      if (editor.__statkissCaptionRefreshing) return;
      var needs = mutations.some(function(m){ return m.type === 'childList' && (m.addedNodes.length || m.removedNodes.length); });
      if (needs) scheduleCaptionRefresh(editor);
    });
    observer.observe(editor.editor, { childList: true, subtree: true });
    editor.__statkissCaptionObserver = observer;
  }

  function findFieldContainer(input){
    if (!input || !input.closest) return null;
    return input.closest('label, .lre-field, .inspector-grid > div, .inspector-grid > label');
  }
  ...
  function replaceInputWithoutListeners(input){
  ...
  function createCaptionField(id){
    var label = document.createElement('label');
    label.className = 'lre-field statkiss-caption-field';
    label.innerHTML = 'Caption <input id="' + id + '" type="text" placeholder="Caption">';
    return label;
  }

  function bindCaptionInput(editor, kind, input){
    if (!input || input.__statkissCaptionBound) return input;
    input.__statkissCaptionBound = true;
    input.addEventListener('input', function(){
      var node = getSelectedNode(editor);
      if (!node || getNodeKind(node) !== kind) return;
      var base = String(input.value || '').trim();
      setCaptionState(node, !!base, base);
      refreshCaptions(editor);
      if (isFn(editor.markDirty)) editor.markDirty();
      if (isFn(editor.updateSourceView)) editor.updateSourceView();
      if (kind === 'image' && isFn(editor.populateImageInspector)) editor.populateImageInspector(node);
    });
    return input;
  }

  function ensureSectionCaptionInput(section, id, beforeSelectors){
    if (!section) return null;
    var input = q(section, '#' + id);
    if (!input) {
      var field = createCaptionField(id);
      var before = null;
      (beforeSelectors || []).some(function(sel){ before = q(section, sel); return !!before; });
      if (before && before.parentNode) before.parentNode.insertBefore(field, before);
      else {
        var h3 = q(section, 'h3');
        if (h3 && h3.nextSibling) section.insertBefore(field, h3.nextSibling);
        else section.appendChild(field);
      }
      input = q(field, 'input');
    }
    if (input) {
      setFieldLabel(input, 'Caption');
      input.setAttribute('placeholder', 'Caption');
    }
    return input;
  }

  function ensureCaptionInputs(editor){
    if (!editor || !editor.root) return;
    editor.__statkissCaptionInputs = editor.__statkissCaptionInputs || {};

    var imageSection = editor.imageInspector || q(editor.root, '#lreImageInspector') || q(editor.root, '#imageInspector');
    if (imageSection) {
      var imageInput = q(imageSection, '#lreImageCaption') || q(imageSection, '#imageCaptionInput');
      if (imageInput && !imageInput.__statkissCaptionListenerReset) {
        imageInput = replaceInputWithoutListeners(imageInput);
        imageInput.__statkissCaptionListenerReset = true;
      }
      if (imageInput) {
        setFieldLabel(imageInput, 'Caption');
        imageInput.setAttribute('placeholder', 'Caption');
        editor.imageCaption = imageInput;
        editor.imageCaptionInput = imageInput;
        editor.__statkissCaptionInputs.image = bindCaptionInput(editor, 'image', imageInput);
      }
      var altInput = q(imageSection, '#lreImageAlt') || q(imageSection, '#imageAltInput');
      if (altInput) {
        var altField = findFieldContainer(altInput) || altInput;
        if (altField) altField.remove();
      }
      editor.imageAlt = { value: '' };
      editor.imageAltInput = editor.imageAlt;
    }

    var tableSection = editor.tableInspector || q(editor.root, '#lreTableInspector') || q(editor.root, '#tableInspector');
    if (tableSection) {
      var tableInput = ensureSectionCaptionInput(tableSection, 'statkissTableCaptionInput', ['#lreTableInfo', '#tableInfo', '#tableLocation', '.inspector-button-grid', '.lre-actions']);
      editor.__statkissCaptionInputs.table = bindCaptionInput(editor, 'table', tableInput);
    }

    var mathSection = editor.mathInspector || editor.latexInspector || q(editor.root, '#lreMathInspector') || q(editor.root, '#latexInspector');
    if (mathSection) {
      var mathInput = ensureSectionCaptionInput(mathSection, 'statkissMathCaptionInput', ['#lreMathText', '#latexContent', 'textarea', '.lre-field', 'p', 'button']);
      editor.__statkissCaptionInputs.math = bindCaptionInput(editor, 'math', mathInput);
    }

    var codeSection = editor.codeInspector || q(editor.root, '#lreCodeInspector') || q(editor.root, '#codeInspector');
    if (codeSection) {
      var codeInput = ensureSectionCaptionInput(codeSection, 'statkissCodeCaptionInput', ['label', '.inspector-grid label', '#codeInfo', '.lre-actions']);
      editor.__statkissCaptionInputs.code = bindCaptionInput(editor, 'code', codeInput);
    }
  }

  function syncCaptionInputs(editor){
    if (!editor) return;
    ensureCaptionInputs(editor);
    var node = getSelectedNode(editor);
    var kind = getNodeKind(node);
    var inputs = editor.__statkissCaptionInputs || {};
    if (inputs.image) inputs.image.value = kind === 'image' ? getCaptionBase(node, 'image') : '';
    if (inputs.table) inputs.table.value = kind === 'table' ? getCaptionBase(node, 'table') : '';
    if (inputs.math) inputs.math.value = kind === 'math' ? getCaptionBase(node, 'math') : '';
    if (inputs.code) inputs.code.value = kind === 'code' ? getCaptionBase(node, 'code') : '';
  }

  function refreshInspector(editor){
    cleanupInspectorHeader(editor);
    ensureCaptionInputs(editor);
    syncCaptionInputs(editor);
    var selected = getSelectedNode(editor);
    if (!selected) {
      hideInspector(editor, false);
      return;
    }
    if (editor.__statkissManualInspectorHidden) {
      var panel = getInspectorPanel(editor);
  ...
  function setNodeLock(editor, node){
    editor.__statkissNodeLock = { node: node, until: Date.now() + 260 };
  }

  function getNodeLock(editor){
    var lock = editor && editor.__statkissNodeLock;
    if (!lock) return null;
    if (lock.until > Date.now() && lock.node && document.contains(lock.node)) return lock.node;
    editor.__statkissNodeLock = null;
    return null;
  }

  function patchEditor(editor){
    if (!editor || editor.__STATKISS_ANNOUNCEMENT_REFINEMENT_PATCHED__) return editor;
    editor.__STATKISS_ANNOUNCEMENT_REFINEMENT_PATCHED__ = true;
    ensureStyle();
    ensureCaptionInputs(editor);
    ensureCaptionObserver(editor);

    editor.updateSelectedImageCaption = function(){
      ensureCaptionInputs(this);
      var input = this.__statkissCaptionInputs && this.__statkissCaptionInputs.image;
      if (input) input.dispatchEvent(new Event('input', { bubbles: true }));
    };
    editor.updateSelectedImageAlt = function() { return; };
    editor.updateSelectedImageWidth = function(){
      var node = isFn(this.getSelectedImageNode) ? this.getSelectedImageNode() : null;
      var image = node && q(node, 'img');
      var input = this.imageWidthRange || this.imageWidth || q(this.root, '#imageWidthRange') || q(this.root, '#lreImageWidth');
      var label = this.imageWidthLabel || q(this.root, '#imageWidthLabel') || this.imageWidthLabel || q(this.root, '#lreImageWidthLabel');
      if (!node || !image || !input) return;
      var value = Math.max(20, Math.min(100, parseInt(input.value || '100', 10) || 100));
      image.style.width = value + '%';
      if (label) label.textContent = value + '%';
      if (isFn(this.markDirty)) this.markDirty();
      if (isFn(this.updateSourceView)) this.updateSourceView();
    };
    editor.updateSelectedImageAlign = function(){
      var node = isFn(this.getSelectedImageNode) ? this.getSelectedImageNode() : null;
      if (!node) return;
      var value = this.imageAlignSelect && this.imageAlignSelect.value ? this.imageAlignSelect.value : 'center';
      node.classList.remove('align-left', 'align-center', 'align-right');
      node.classList.add('align-' + value);
      qa((this.imageInspector || this.root), '[data-image-align]').forEach(function(btn){ btn.classList.toggle('is-active', btn.getAttribute('data-image-align') === value); });
      if (isFn(this.markDirty)) this.markDirty();
      if (isFn(this.updateSourceView)) this.updateSourceView();
    };
    ...
    var oldPopulateImageInspector = isFn(editor.populateImageInspector) ? editor.populateImageInspector.bind(editor) : null;
    editor.populateImageInspector = function(node){
      if (oldPopulateImageInspector) oldPopulateImageInspector(node);
      ensureCaptionInputs(this);
      syncCaptionInputs(this);
      var image = node && q(node, 'img');
      var widthInput = this.imageWidthRange || this.imageWidth || q(this.root, '#imageWidthRange') || q(this.root, '#lreImageWidth');
      var widthLabel = this.imageWidthLabel || q(this.root, '#imageWidthLabel') || q(this.root, '#lreImageWidthLabel');
      if (image && widthInput) {
        var width = Math.max(20, Math.min(100, parseInt(image.style.width || widthInput.value || '100', 10) || 100));
        widthInput.value = String(width);
        if (widthLabel) widthLabel.textContent = width + '%';
      }
      if (this.imageAlignSelect) {
        var align = node && node.classList && node.classList.contains('align-left') ? 'left' : node && node.classList && node.classList.contains('align-right') ? 'right' : 'center';
        this.imageAlignSelect.value = align;
        qa((this.imageInspector || this.root), '[data-image-align]').forEach(function(btn){ btn.classList.toggle('is-active', btn.getAttribute('data-image-align') === align); });
      }
    };

    var oldPopulateTableInspector = isFn(editor.populateTableInspector) ? editor.populateTableInspector.bind(editor) : null;
    editor.populateTableInspector = function(table){
      if (oldPopulateTableInspector) oldPopulateTableInspector(table);
      ensureCaptionInputs(this);
      syncCaptionInputs(this);
    };

    var oldPopulateMathInspector = isFn(editor.populateMathInspector) ? editor.populateMathInspector.bind(editor) : null;
    editor.populateMathInspector = function(node){
      if (oldPopulateMathInspector) oldPopulateMathInspector(node);
      ensureCaptionInputs(this);
      syncCaptionInputs(this);
    };

    var oldPopulateCodeInspector = isFn(editor.populateCodeInspector) ? editor.populateCodeInspector.bind(editor) : null;
    editor.populateCodeInspector = function(node){
      if (oldPopulateCodeInspector) oldPopulateCodeInspector(node);
      ensureCaptionInputs(this);
      syncCaptionInputs(this);
    };
    ...
    editor.selectNode = function(node){
      if (!node) return;
      if (getNodeKind(node) === 'math') node = normalizeMathNode(node);
      setNodeLock(this, node);
      this.__statkissManualInspectorHidden = false;
      var result = oldSelectNode ? oldSelectNode(node) : (this.selectedNode = node);
      ensureStatePointers(this, node);
      refreshInspector(this);
      return result;
    };

    var oldClearNodeSelection = isFn(editor.clearNodeSelection) ? editor.clearNodeSelection.bind(editor) : null;
    editor.clearNodeSelection = function(force){
      if (!force && getNodeLock(this)) return;
      var result = oldClearNodeSelection ? oldClearNodeSelection() : (this.selectedNode = null);
      if (!getSelectedNode(this)) hideInspector(this, false);
      syncCaptionInputs(this);
      return result;
    };

    var oldDetectSelectedNode = isFn(editor.detectSelectedNode) ? editor.detectSelectedNode.bind(editor) : null;
    editor.detectSelectedNode = function(){
      var locked = getNodeLock(this);
      if (locked) {
        if (this.selectedNode !== locked) {
          ensureStatePointers(this, locked);
          this.selectedNode = locked;
          locked.classList && locked.classList.add('is-selected');
        }
        refreshInspector(this);
        return locked;
      }
      var result = oldDetectSelectedNode ? oldDetectSelectedNode() : null;
      refreshInspector(this);
      return result;
    };

    var oldUpdateSelectedContext = isFn(editor.updateSelectedContextFromSelection) ? editor.updateSelectedContextFromSelection.bind(editor) : null;
    editor.updateSelectedContextFromSelection = function(){
      if (getNodeLock(this)) return;
      if (oldUpdateSelectedContext) return oldUpdateSelectedContext();
    };

    var oldHandleEditorClick = isFn(editor.handleEditorClick) ? editor.handleEditorClick.bind(editor) : null;
    editor.handleEditorClick = function(event){
      var node = getInspectableNode(event && event.target);
      if (node) { this.selectNode(node); return; }
      if (oldHandleEditorClick) return oldHandleEditorClick(event);
      this.clearNodeSelection(true);
    };

    if (!editor.__statkissStableInspectorBound && editor.editor) {
      editor.__statkissStableInspectorBound = true;
      editor.editor.addEventListener('mousedown', function(ev){
        var node = getInspectableNode(ev.target);
        if (!node) return;
        setNodeLock(editor, node);
        if (getNodeKind(node) !== 'table') ev.preventDefault();
      }, true);
      editor.editor.addEventListener('click', function(ev){
        var node = getInspectableNode(ev.target);
        if (node) {
          setNodeLock(editor, node);
        ...
        if (!ev.target.closest('#lreInspectorPanel, #inspectorPanel, .lre-dialog, dialog')) {
          setTimeout(function(){ editor.clearNodeSelection(true); }, 0);
        }
      }, true);
      document.addEventListener('selectionchange', function(){
        if (!editor.root || !editor.root.isConnected) return;
        if (getNodeLock(editor)) return;
        var sel = document.getSelection ? document.getSelection() : null;
        if (sel && sel.anchorNode && editor.root.contains(sel.anchorNode)) refreshInspector(editor);
      }, true);
    }

    var oldDeleteSelectedNode = isFn(editor.deleteSelectedNode) ? editor.deleteSelectedNode.bind(editor) : null;
    if (oldDeleteSelectedNode) {
      editor.deleteSelectedNode = function(){
        var result = oldDeleteSelectedNode();
        setTimeout(function(){ refreshCaptions(editor); refreshInspector(editor); if (isFn(editor.updateSourceView)) editor.updateSourceView(); }, 0);
        return result;
      };
    }

    var oldInsertImageFile = isFn(editor.insertImageFile) ? editor.insertImageFile.bind(editor) : null;
    if (oldInsertImageFile) {
      editor.insertImageFile = function(file){
        var result = oldInsertImageFile(file);
        if (result && isFn(result.then)) {
          return result.then(function(value){ setTimeout(function(){ refreshCaptions(editor); refreshInspector(editor); }, 0); return value; });
        }
        setTimeout(function(){ refreshCaptions(editor); refreshInspector(editor); }, 0);
        return result;
      };
    }

    var oldSaveMathDialog = isFn(editor.saveMathDialog) ? editor.saveMathDialog.bind(editor) : null;
    if (oldSaveMathDialog) {
      editor.saveMathDialog = function(){
        var result = oldSaveMathDialog();
        setTimeout(function(){ refreshCaptions(editor); refreshInspector(editor); if (isFn(editor.updateSourceView)) editor.updateSourceView(); }, 0);
        return result;
      };
    }

    var oldSaveCodeDialog = isFn(editor.saveCodeDialog) ? editor.saveCodeDialog.bind(editor) : null;
    if (oldSaveCodeDialog) {
      editor.saveCodeDialog = function(){
        var result = oldSaveCodeDialog();
        setTimeout(function(){ refreshCaptions(editor); refreshInspector(editor); if (isFn(editor.updateSourceView)) editor.updateSourceView(); }, 0);
        return result;
      };
    }

    var oldSetHTML = isFn(editor.setHTML) ? editor.setHTML.bind(editor) : null;
    if (oldSetHTML) {
      editor.setHTML = function(html){
        var result = oldSetHTML(html);
        setTimeout(function(){ refreshCaptions(editor); refreshInspector(editor); }, 0);
        return result;
      };
    }

    var oldSetMarkdown = isFn(editor.setMarkdown) ? editor.setMarkdown.bind(editor) : null;
    if (oldSetMarkdown) {
      editor.setMarkdown = function(markdown){
        var result = oldSetMarkdown(markdown);
        setTimeout(function(){ refreshCaptions(editor); refreshInspector(editor); }, 0);
        return result;
      };
    }

    var oldNormalizeAfterImport = isFn(editor.normalizeAfterImport) ? editor.normalizeAfterImport.bind(editor) : null;
    if (oldNormalizeAfterImport) {
      editor.normalizeAfterImport = function(){
        var result = oldNormalizeAfterImport();
        setTimeout(function(){ refreshCaptions(editor); refreshInspector(editor); }, 0);
        return result;
      };
    }
  ...
  function wrapMount(name){
    ...
    if (!isFn(original) || original.__statkissAnnouncementRefinementWrapped) return;
    var wrapped = function(){
      var result = original.apply(this, arguments);
      if (result && isFn(result.then)) return result.then(function(editor){ return patchEditor(editor); });
      return patchEditor(result);
    };
    wrapped.__statkissAnnouncementRefinementWrapped = true;
    window[name] = wrapped;
  }
  ...
  log('statkiss-announcement-refinement-20260403-02 applied');
})();

/* StatKISS 20260403 final codeblock duplication + unicode fallback fix */
(function(){
  'use strict';

  var PATCH_ID = 'statkiss-codeblock-dup-unicode-fix-20260403-01';
  if (window.__STATKISS_CODEBLOCK_DUP_UNICODE_FIX__ === PATCH_ID) return;
  window.__STATKISS_CODEBLOCK_DUP_UNICODE_FIX__ = PATCH_ID;
  ...
  function wrapMount(name, patch){
    var original = window[name];
    if (!isFn(original) || original.__statkissCodeDupUnicodeWrapped) return;
    var wrapped = function(){
      var result = original.apply(this, arguments);
      if (result && isFn(result.then)) {
        return result.then(function(editor){
          return patch(editor) || editor;
        });
      }
      return patch(result) || result;
    };
    wrapped.__statkissCodeDupUnicodeWrapped = true;
    wrapped.__statkissCodeDupUnicodeOriginal = original;
    window[name] = wrapped;
  }

  function getCodeFigure(node){
    if (!node) return null;
    if (node.classList && node.classList.contains('lre-code-node')) return node;
    return node.closest ? node.closest('.lre-code-node') : null;
  }

  function ensureCodeDom(node){
    if (!node) return null;
    var head = q(node, '.lre-code-head');
    if (!head) {
      head = document.createElement('div');
      head.className = 'lre-code-head';
      node.insertBefore(head, node.firstChild || null);
    }
    var chip = q(head, '.lre-chip');
    if (!chip) {
      chip = document.createElement('span');
      chip.className = 'lre-chip';
      head.insertBefore(chip, head.firstChild || null);
    }
    var pre = q(node, 'pre');
    if (!pre) {
      pre = document.createElement('pre');
    ...
    return { head: head, chip: chip, pre: pre, code: code };
  }

  function renderCodeWithHljs(text, language){
    var raw = String(text == null ? '' : text);
    var requested = normalizeLang(language);
    var hljs = window.hljs;
    if (!hljs || !isFn(hljs.highlight)) {
      return { language: requested === 'auto' ? 'plaintext' : requested, plain: true, html: '' };
    }
    var mapped = requested;
    if (!mapped || mapped === 'auto') {
      try {
        if (isFn(hljs.highlightAuto)) {
          var autoRes = hljs.highlightAuto(raw);
          return {
            language: autoRes && autoRes.language ? autoRes.language : 'plaintext',
            plain: false,
            html: autoRes && autoRes.value ? autoRes.value : esc(raw)
          };
        }
      } catch (e) {}
      return { language: 'plaintext', plain: true, html: '' };
    }
    if (mapped === 'nohighlight' || mapped === 'plaintext') {
      return { language: 'plaintext', plain: true, html: '' };
    }
    try {
      if (isFn(hljs.getLanguage) && hljs.getLanguage(mapped)) {
        var result = hljs.highlight(raw, { language: mapped, ignoreIllegals: true });
        return { language: mapped, plain: false, html: result && result.value ? result.value : esc(raw) };
      }
    } catch (e) {}
    return { language: mapped || 'plaintext', plain: true, html: '' };
  }

  function paintCodeNode(editor, node){
    var figure = getCodeFigure(node);
    if (!figure) return null;
    var dom = ensureCodeDom(figure);
    if (!dom) return null;

    var raw = String(
      figure.dataset.code != null ? figure.dataset.code :
      (dom.code.dataset.rawCode != null ? dom.code.dataset.rawCode : dom.code.textContent || '')
    );
    var requested = normalizeLang(figure.dataset.language || dom.code.dataset.language || dom.pre.dataset.language || 'plaintext');
    var rendered = renderCodeWithHljs(raw, requested);
    var finalLang = normalizeLang(rendered.language || requested || 'plaintext');

    figure.dataset.code = raw;
    figure.dataset.language = finalLang;
    dom.chip.textContent = finalLang;
    dom.pre.dataset.language = finalLang;
    dom.code.dataset.rawCode = raw;
    dom.code.dataset.language = finalLang;
    dom.code.setAttribute('spellcheck', 'false');
    dom.code.setAttribute('contenteditable', 'false');

    if (rendered.plain) {
      dom.code.textContent = raw;
      dom.code.className = 'hljs language-plaintext';
    } else {
      dom.code.innerHTML = rendered.html;
      dom.code.className = 'hljs language-' + finalLang;
    }

    if (editor && isFn(editor.populateCodeInspector) && isFn(editor.getSelectedCodeNode) && editor.getSelectedCodeNode() === figure) {
      try { editor.populateCodeInspector(figure); } catch (e) {}
    }
    return figure;
  }

  function stopDialogBubble(dialog){
    if (!dialog || dialog.__statkissCodeDialogBubbleStopped) return;
    dialog.__statkissCodeDialogBubbleStopped = true;
    dialog.addEventListener('click', function(ev){
      var target = ev.target;
      if (!target || !target.closest) return;
      if (target.closest('[data-action="save-code-dialog"], [data-action="save-math-dialog"], [data-close-dialog]')) {
        ev.stopPropagation();
      }
    });
  }

  function installSaveGuards(editor){
    if (!editor || editor.__statkissCodeSaveGuardsInstalled) return;
    editor.__statkissCodeSaveGuardsInstalled = true;

    if (isFn(editor.saveCodeDialog)) {
      var originalSaveCodeDialog = editor.saveCodeDialog.bind(editor);
      editor.saveCodeDialog = function(){
        var startedAt = Date.now();
        if (this.__statkissCodeDialogBusy && (startedAt - this.__statkissCodeDialogBusy) < 300) return;
        this.__statkissCodeDialogBusy = startedAt;
        try {
          var result = originalSaveCodeDialog();
          var node = isFn(this.getSelectedCodeNode) ? this.getSelectedCodeNode() : null;
          if (node) paintCodeNode(this, node);
          return result;
        } finally {
          var self = this;
          setTimeout(function(){
            if (self.__statkissCodeDialogBusy === startedAt) self.__statkissCodeDialogBusy = 0;
          }, 300);
        }
      };
    }

    if (isFn(editor.saveMathDialog)) {
      var originalSaveMathDialog = editor.saveMathDialog.bind(editor);
      editor.saveMathDialog = function(){
        var startedAt = Date.now();
        if (this.__statkissMathDialogBusy && (startedAt - this.__statkissMathDialogBusy) < 300) return;
        this.__statkissMathDialogBusy = startedAt;
        try {
          return originalSaveMathDialog();
        } finally {
          var self = this;
          setTimeout(function(){
            if (self.__statkissMathDialogBusy === startedAt) self.__statkissMathDialogBusy = 0;
          }, 300);
        }
      };
    }
  ...
  function patchInstance(editor){
    if (!editor || editor.__statkissCodeDupUnicodePatched) return editor;
    editor.__statkissCodeDupUnicodePatched = true;

    stopDialogBubble(editor.codeDialog);
    stopDialogBubble(editor.mathDialog);
    installSaveGuards(editor);

    var originalHandleAction = isFn(editor.handleAction) ? editor.handleAction.bind(editor) : null;
    editor.handleAction = function(action, button){
      if (action === 'save-code-dialog' && this.__statkissCodeDialogBusy) return;
      if (action === 'save-math-dialog' && this.__statkissMathDialogBusy) return;
      return originalHandleAction ? originalHandleAction(action, button) : undefined;
    };
    ...
    editor.renderCodeNode = function(node){
      if (originalRenderCodeNode) originalRenderCodeNode(node);
      return paintCodeNode(this, node);
    };

    editor.highlightCodeBlocksInside = function(root){
      var scope = root || this.editor;
      qa(scope, '.lre-code-node').forEach(function(node){ paintCodeNode(editor, node); });
    };

    editor.highlightAllCodeBlocks = function(){
      this.highlightCodeBlocksInside(this.editor);
      if (this.sourcePreview) this.highlightCodeBlocksInside(this.sourcePreview);
    };

    if (isFn(editor.refreshSelectedCode)) {
      var originalRefreshSelectedCode = editor.refreshSelectedCode.bind(editor);
      editor.refreshSelectedCode = function(){
        var result = originalRefreshSelectedCode();
        var node = isFn(this.getSelectedCodeNode) ? this.getSelectedCodeNode() : null;
        if (node) paintCodeNode(this, node);
        return result;
      };
    ...
    if (isFn(editor.refreshSelectedCodeBlock)) {
      var originalRefreshSelectedCodeBlock = editor.refreshSelectedCodeBlock.bind(editor);
      editor.refreshSelectedCodeBlock = function(){
        var result = originalRefreshSelectedCodeBlock();
        var node = isFn(this.getSelectedCodeNode) ? this.getSelectedCodeNode() : null;
        if (node) paintCodeNode(this, node);
        return result;
      };
    ...
    if (isFn(editor.normalizeAfterImport)) {
      var originalNormalizeAfterImport = editor.normalizeAfterImport.bind(editor);
      editor.normalizeAfterImport = function(){
        var result = originalNormalizeAfterImport();
...
/* Generic host-facing aliases for the patched editor runtime */
(function(){
  'use strict';
  if (typeof window === 'undefined') return;

  function callLatest(primaryName, fallbackName) {
    return function() {
      var primary = window[primaryName];
      if (typeof primary === 'function') return primary.apply(this, arguments);
      if (fallbackName && typeof window[fallbackName] === 'function') return window[fallbackName].apply(this, arguments);
      return null;
    };
  }

  window.mountContentEditor = callLatest('mountStatkissContentEditor', 'mountEmbeddedContentEditor');
  window.autoMountContentEditor = callLatest('autoMountStatkissContentEditor');
  window.initContentEditor = callLatest('initStatkissContentEditor');
  window.renderContentWriteDemo = callLatest('renderStatkissContentWriteDemo');
})();


/* Generic 20260404 code highlight visual + auto-detect fix */
(function(){
  if (window.__CONTENT_EDITOR_CODE_HIGHLIGHT_VISUAL_FIX_20260404__) return;
  window.__CONTENT_EDITOR_CODE_HIGHLIGHT_VISUAL_FIX_20260404__ = true;
  ...
  function normLang(value){
    ...
    var aliases = {
      ...
      yml: 'yaml',
      md: 'markdown',
      htm: 'html', xhtml: 'html', xml: 'html', htmlmixed: 'html',
      text: 'plaintext', txt: 'plaintext', plain: 'plaintext'
    };
    return aliases[raw] || raw || 'plaintext';
  }
  function ensureStyle(){
    if (document.getElementById('content-editor-code-highlight-visual-fix-20260404')) return;
    var style = document.createElement('style');
    style.id = 'content-editor-code-highlight-visual-fix-20260404';
    style.textContent = [
      '.lre-root .lre-code-node code.hljs, .lre-root .lre-source-preview code.hljs{display:block;background:transparent !important;color:var(--lre-code-fg,#10213f) !important;}',
      '.lre-root .lre-code-node pre, .lre-root .lre-source-preview pre{background:transparent !important;}',
      '.lre-root .hljs-comment{color:var(--tok-comment,#64748b) !important;font-style:italic;}',
      '.lre-root .hljs-quote{color:var(--tok-comment,#64748b) !important;font-style:italic;}',
      '.lre-root .hljs-keyword{color:var(--tok-keyword,#7c3aed) !important;font-weight:700;}',
      '.lre-root .hljs-selector-tag{color:var(--tok-keyword,#7c3aed) !important;font-weight:700;}',
      '.lre-root .hljs-literal{color:var(--tok-keyword,#7c3aed) !important;font-weight:700;}',
      '.lre-root .hljs-string{color:var(--tok-string,#0f766e) !important;}',
      '.lre-root .hljs-number{color:var(--tok-number,#b45309) !important;}',
      '.lre-root .hljs-attr{color:var(--tok-attr,#1d4ed8) !important;}',
      '.lre-root .hljs-attribute{color:var(--tok-attr,#1d4ed8) !important;}',
      '.lre-root .hljs-tag{color:var(--tok-tag,#c2410c) !important;}',
      '.lre-root .hljs-name{color:var(--tok-tag,#c2410c) !important;}',
      '.lre-root .hljs-title{color:var(--tok-function,#1d4ed8) !important;}',
      '.lre-root .hljs-title.function_, .lre-root .hljs-title.class_{color:var(--tok-function,#1d4ed8) !important;}',
      '.lre-root .hljs-built_in{color:var(--tok-builtin,#be185d) !important;}',
      '.lre-root .hljs-property{color:var(--tok-prop,#0f766e) !important;}',
      '.lre-root .hljs-variable{color:var(--tok-var,#9333ea) !important;}',
      '.lre-root .hljs-template-variable{color:var(--tok-var,#9333ea) !important;}',
      '.lre-root .hljs-regexp{color:var(--tok-emph,#be185d) !important;}',
      '.lre-root .hljs-link{color:var(--tok-function,#1d4ed8) !important;text-decoration:underline;}',
      '.lre-root .hljs-bullet{color:var(--tok-keyword,#7c3aed) !important;}',
      '.lre-root .hljs-operator{color:var(--tok-operator,#334155) !important;}',
      '.lre-root .hljs-punctuation{color:var(--tok-punct,#475569) !important;}',
      '.lre-root .hljs-subst{color:var(--lre-code-fg,#10213f) !important;}',
      '.lre-root .lre-code-node .lre-chip[data-auto="true"]::after{content:" · auto";opacity:.72;font-size:11px;font-weight:600;}'
    ].join('');
    document.head.appendChild(style);
  }
  ...
  function ensureAutoOptions(editor){
    if (!editor) return;
    ensureAutoOption(editor.codeDialogLang || q(editor.root, '#lreCodeDialogLang'));
    ensureAutoOption(editor.codeLanguageSelect || q(editor.root, '#lreCodeLanguageSelect'));
  }
  function wordRegex(words){
    return new RegExp('\\b(' + words.map(function(word){ return String(word).replace(/[.*+?^${}()|[\\]\\]/g, '\\$&'); }).join('|') + ')\\b', 'g');
  }
  var JS_KEYWORDS = wordRegex(['break','case','catch','class','const','continue','default','delete','do','else','export','extends','finally','for','from','function','if','import','in','instanceof','let','new','of','return','switch','throw','try','typeof','var','void','while','with','yield','async','await','static','this','super','true','false','null','undefined']);
  var JS_BUILTINS = wordRegex(['Array','Boolean','Date','Error','Function','JSON','Map','Math','Number','Object','Promise','RegExp','Set','String','Symbol','console','document','window','globalThis','fetch']);
  var PY_KEYWORDS = wordRegex(['and','as','assert','async','await','break','class','continue','def','del','elif','else','except','False','finally','for','from','global','if','import','in','is','lambda','None','nonlocal','not','or','pass','raise','return','True','try','while','with','yield']);
  var BASH_KEYWORDS = wordRegex(['if','then','else','fi','for','in','do','done','case','esac','function','echo','export','local','readonly','while','until','return']);
  var SQL_KEYWORDS = wordRegex(['select','from','where','join','left','right','inner','outer','on','group','by','order','insert','into','values','update','set','delete','create','table','alter','drop','as','and','or','not','null','limit','having','distinct']);
  var CSS_KEYWORDS = wordRegex(['display','position','color','background','font','padding','margin','border','width','height','grid','flex','absolute','relative','fixed','sticky','block','inline','none']);

  function stashMatches(text, defs){
    var src = String(text || '');
    var stash = [];
    defs.forEach(function(def){
      src = src.replace(def.regex, function(match){
        var key = '\uE000' + stash.length + '\uE001';
        stash.push('<span class="' + def.className + '">' + esc(match) + '</span>');
        return key;
      });
    });
    return {
      src: src,
      restore: function(html){
        return html.replace(/\uE000(\d+)\uE001/g, function(_, idx){ return stash[Number(idx)] || ''; });
      }
    };
  }
  function detectLanguage(text){
    var value = String(text || '');
    if (/^\s*</.test(value) && /<\/?[A-Za-z!][^>]*>/.test(value)) return 'html';
    if (/^\s*[\[{]/.test(value) && /:\s*/.test(value) && /["']?[A-Za-z0-9_\-]+["']?\s*:/.test(value)) return 'json';
    if (/^\s*(SELECT|INSERT|UPDATE|DELETE|CREATE|ALTER|DROP)\b/i.test(value)) return 'sql';
    if (/^\s*#?!\/(usr\/bin\/env\s+)?(ba)?sh\b/m.test(value) || /\bfi\b|\bthen\b|\becho\b/.test(value)) return 'bash';
    if (/^\s*def\s+\w+|^\s*class\s+\w+|\bimport\s+\w+/m.test(value)) return 'python';
    if (/^\s*[.#]?[A-Za-z_][\w-]*\s*\{/.test(value) || /^[^{\n]+\{[\s\S]*:[\s\S]*;[\s\S]*\}/m.test(value)) return 'css';
    if (/^\s*#{1,6}\s/m.test(value) || /^\s*[-*+]\s/m.test(value) || /```/.test(value)) return 'markdown';
    if (/\b(function|const|let|var|class|return|import|export)\b/.test(value) || /=>/.test(value)) return 'javascript';
    return 'plaintext';
  }
  function chooseLanguage(requested, text){
    var req = normLang(requested);
    var detected = detectLanguage(text);
    if (!req || req === 'auto') return detected;
    if (req === 'plaintext') return 'plaintext';
    if (req === 'nohighlight') return 'nohighlight';
    if ((req === 'javascript' || req === 'typescript') && detected && detected !== 'javascript' && detected !== 'plaintext') return detected;
    return req;
  }
  function highlightHtmlLike(text){
    var html = esc(text);
  ...
  function highlightCss(text){
    var tok = stashMatches(text, [
      { regex: /\/\*[\s\S]*?\*\//g, className: 'hljs-comment' },
      { regex: /"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'/g, className: 'hljs-string' }
    ]);
    var html = esc(tok.src);
    html = html.replace(/([.#]?[A-Za-z_][\w-]*)(\s*\{)/g, '<span class="hljs-title class_">$1</span>$2');
    html = html.replace(/([A-Za-z-]+)(\s*:)/g, '<span class="hljs-attr">$1</span>$2');
    html = html.replace(CSS_KEYWORDS, '<span class="hljs-keyword">$1</span>');
    html = html.replace(/\b(#[0-9a-fA-F]{3,8}|\d+(?:\.\d+)?(?:px|em|rem|%|vh|vw)?|auto)\b/g, '<span class="hljs-number">$1</span>');
  ...
  function highlightScriptLike(text, options){
    ...
    var keywords = options.keywords || JS_KEYWORDS;
    var builtins = options.builtins || null;
    var defs = comments.map(function(regex){ return { regex: regex, className: 'hljs-comment' }; });
    defs.push({ regex: /`(?:\\[\s\S]|[^`])*`/g, className: 'hljs-string' });
    defs.push({ regex: /"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'/g, className: 'hljs-string' });
    defs.push({ regex: /\/(?![/*])(?:\\.|[^\/\\\n])+\/[dgimsuvy]*/g, className: 'hljs-regexp' });
    var tok = stashMatches(text, defs);
    var html = esc(tok.src);
    html = html.replace(/\b(function|def|class)\s+([A-Za-z_$][\w$]*)/g, '<span class="hljs-keyword">$1</span> <span class="hljs-title function_">$2</span>');
    html = html.replace(keywords, '<span class="hljs-keyword">$1</span>');
    if (builtins) html = html.replace(builtins, '<span class="hljs-built_in">$1</span>');
  ...
  function offlineHighlight(text, language){
    var lang = chooseLanguage(language, text);
    if (lang === 'nohighlight' || lang === 'plaintext') return { language: 'plaintext', html: esc(text), plain: true };
    if (lang === 'html' || lang === 'xml') return { language: 'html', html: highlightHtmlLike(text), plain: false };
    if (lang === 'json') return { language: 'json', html: highlightJson(text), plain: false };
    if (lang === 'css') return { language: 'css', html: highlightCss(text), plain: false };
    if (lang === 'python') return { language: 'python', html: highlightScriptLike(text, { comments: [/#.*$/gm], keywords: PY_KEYWORDS, builtins: wordRegex(['print','len','range','str','int','float','list','dict','set','tuple','enumerate']) }), plain: false };
    if (lang === 'sql') return { language: 'sql', html: highlightScriptLike(text, { comments: [/--[^\n]*/g, /\/\*[\s\S]*?\*\//g], keywords: SQL_KEYWORDS, builtins: wordRegex(['count','sum','avg','min','max']) }), plain: false };
    if (lang === 'bash') return { language: 'bash', html: highlightScriptLike(text, { comments: [/#.*$/gm], keywords: BASH_KEYWORDS, builtins: wordRegex(['printf','echo','cd','pwd','grep','sed','awk','cat']) }), plain: false };
    if (lang === 'markdown') return { language: 'markdown', html: highlightMarkdown(text), plain: false };
    return { language: 'javascript', html: highlightScriptLike(text, { comments: [/\/\*[\s\S]*?\*\//g, /\/\/[^\n]*/g], keywords: JS_KEYWORDS, builtins: JS_BUILTINS }), plain: false };
  }
  function highlightWithHljs(text, language){
    var raw = String(text == null ? '' : text);
    var preferred = chooseLanguage(language, raw);
    var hljs = window.hljs;
    if (!hljs || !isFn(hljs.highlight)) return offlineHighlight(raw, preferred);
    if (preferred === 'nohighlight' || preferred === 'plaintext') return { language: 'plaintext', html: esc(raw), plain: true };
    try {
      if (!preferred || preferred === 'auto') {
        if (isFn(hljs.highlightAuto)) {
          var autoResult = hljs.highlightAuto(raw);
          return {
            language: normLang((autoResult && autoResult.language) || detectLanguage(raw) || 'plaintext'),
            html: autoResult && autoResult.value ? autoResult.value : esc(raw),
            plain: false
          };
        }
        return offlineHighlight(raw, detectLanguage(raw));
      }
      if (isFn(hljs.getLanguage) && hljs.getLanguage(preferred)) {
        var result = hljs.highlight(raw, { language: preferred, ignoreIllegals: true });
        var html = result && result.value ? result.value : esc(raw);
        var tokenCount = (html.match(/<span\b/g) || []).length;
        if (!tokenCount) {
          var detected = detectLanguage(raw);
          if (detected && detected !== preferred && detected !== 'plaintext' && detected !== 'nohighlight') {
            if (isFn(hljs.getLanguage) && hljs.getLanguage(detected)) {
              var detectedResult = hljs.highlight(raw, { language: detected, ignoreIllegals: true });
              var detectedHtml = detectedResult && detectedResult.value ? detectedResult.value : esc(raw);
              if ((detectedHtml.match(/<span\b/g) || []).length > tokenCount) {
                return { language: normLang(detected), html: detectedHtml, plain: false };
              }
            }
            return offlineHighlight(raw, detected);
          }
        }
        return { language: normLang(preferred), html: html, plain: false };
      }
    } catch (e) {}
    return offlineHighlight(raw, preferred);
  }
  function paintCodeNode(editor, node){
    if (!node || !node.classList || !node.classList.contains('lre-code-node')) return null;
    ensureStyle();
    ensureAutoOptions(editor);
    var pre = q(node, 'pre');
    var codeEl = q(node, 'pre code');
    var chip = q(node, '.lre-chip');
    if (!pre || !codeEl) return null;
    var raw = String(node.dataset.code != null ? node.dataset.code : (codeEl.dataset.rawCode != null ? codeEl.dataset.rawCode : (codeEl.textContent || '')));
    var requested = normLang(node.dataset.language || codeEl.dataset.language || pre.dataset.language || 'auto');
    var preserveAuto = requested === 'auto';
    var rendered = highlightWithHljs(raw, requested);
    var finalLang = normLang(rendered.language || requested || 'plaintext');
    node.dataset.code = raw;
    node.dataset.language = preserveAuto ? 'auto' : finalLang;
    node.dataset.renderLanguage = finalLang;
    pre.dataset.language = finalLang;
    codeEl.dataset.rawCode = raw;
    codeEl.dataset.language = finalLang;
    codeEl.setAttribute('spellcheck', 'false');
    codeEl.setAttribute('contenteditable', 'false');
    if (rendered.plain) codeEl.textContent = raw;
    else codeEl.innerHTML = rendered.html;
    codeEl.className = 'hljs language-' + (rendered.plain ? 'plaintext' : finalLang);
    if (chip) {
      chip.textContent = finalLang;
      chip.dataset.auto = preserveAuto ? 'true' : 'false';
    }
    if (editor && isFn(editor.populateCodeInspector) && isFn(editor.getSelectedCodeNode) && editor.getSelectedCodeNode() === node) {
      try { editor.populateCodeInspector(node); } catch (e) {}
    }
    return node;
  }
  function patchEditor(editor){
    if (!editor || editor.__contentEditorCodeHighlightVisualFixPatched) return editor;
    ensureStyle();
    ensureAutoOptions(editor);
    editor.__contentEditorCodeHighlightVisualFixPatched = true;

    if (isFn(editor.openCodeDialog)) {
      var originalOpenCodeDialog = editor.openCodeDialog.bind(editor);
      editor.openCodeDialog = function(language, code){
        ensureAutoOptions(this);
        var nextLanguage = (typeof language === 'undefined' || language == null || String(language).trim() === '') ? 'auto' : normLang(language);
        if (!code && nextLanguage === 'javascript') nextLanguage = 'auto';
        return originalOpenCodeDialog(nextLanguage, code);
      };
    }

    if (isFn(editor.renderCodeNode)) {
      var originalRenderCodeNode = editor.renderCodeNode.bind(editor);
      editor.renderCodeNode = function(node){
        var result = originalRenderCodeNode(node);
        paintCodeNode(this, node);
        return result;
      };
    }

    editor.highlightCodeBlocksInside = function(root){
      qa(root || this.editor, '.lre-code-node').forEach(function(node){ paintCodeNode(editor, node); });
    };
    editor.highlightAllCodeBlocks = function(){
      this.highlightCodeBlocksInside(this.editor);
      if (this.sourcePreview) this.highlightCodeBlocksInside(this.sourcePreview);
    ...
    if (isFn(editor.saveCodeDialog)) {
      var originalSaveCodeDialog = editor.saveCodeDialog.bind(editor);
      editor.saveCodeDialog = function(){
        if (this.codeDialogLang && !this.codeDialogLang.value) this.codeDialogLang.value = 'auto';
        var result = originalSaveCodeDialog();
        var selected = isFn(this.getSelectedCodeNode) ? this.getSelectedCodeNode() : null;
        if (selected) paintCodeNode(this, selected);
        return result;
      };
    }
    ...
    editor.refreshSelectedCodeBlock = editor.refreshSelectedCode;

    editor.highlightAllCodeBlocks();
    log('content-editor-code-highlight-visual-fix-20260404 applied');
    return editor;
  }
  function waitForEditor(applyFn){
    var tries = 0;
    (function tick(){
      if (window.localRichEditor) { applyFn(window.localRichEditor); return; }
      tries += 1;
      if (tries > 120) return;
      setTimeout(tick, 120);
    })();
  ...
  function wrapMount(name){
    var fn = window[name];
    if (typeof fn !== 'function' || fn.__contentEditorCodeHighlightVisualFixWrapped) return;
    var wrapped = function(){
      var result = fn.apply(this, arguments);
      if (result && typeof result.then === 'function') {
        return result.then(function(editor){ return patchEditor(editor); });
    ...
    wrapped.__contentEditorCodeHighlightVisualFixWrapped = true;
    window[name] = wrapped;
  }

  ensureStyle();
  ['mountStatkissContentEditor','mountEmbeddedContentEditor','initStatkissContentEditor','initEmbeddedContentEditor','autoMountStatkissContentEditor','mountContentEditor','initContentEditor','autoMountContentEditor'].forEach(wrapMount);
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function(){ waitForEditor(patchEditor); }, { once: true });
  } else {
    waitForEditor(patchEditor);
...
/* StatKISS embedded editor dark-mode palette patch 20260403 */
(function(){
  'use strict';

  const PATCH_FLAG = '__statkissEmbeddedDarkPalettePatch20260403';
  const STYLE_ID = 'statkiss-embedded-dark-palette-style-20260403';
  const WATCH_ATTRS = ['class', 'data-theme', 'theme', 'data-bs-theme', 'data-color-mode', 'style'];

  const STYLE = `
    .lre-root.lre-embedded-content {
      --sk-embed-surface: #ffffff;
      --sk-embed-surface-soft: #f8fafc;
      --sk-embed-toolbar: rgba(255,255,255,.96);
      --sk-embed-border: #d7dde7;
      --sk-embed-border-strong: #cbd5e1;
      --sk-embed-text: #111827;
      --sk-embed-text-soft: #475569;
      --sk-embed-muted: #6b7280;
      --sk-embed-accent: #2563eb;
      --sk-embed-accent-soft: rgba(37,99,235,.12);
      --sk-embed-hover: rgba(15,23,42,.05);
      --sk-embed-overlay: rgba(15,23,42,.18);
      --sk-embed-shadow: 0 18px 42px rgba(15,23,42,.12);
    }

    html.dark .lre-root.lre-embedded-content,
    body.dark .lre-root.lre-embedded-content,
    html.theme-dark .lre-root.lre-embedded-content,
    body.theme-dark .lre-root.lre-embedded-content,
    html[data-theme="dark"] .lre-root.lre-embedded-content,
    body[data-theme="dark"] .lre-root.lre-embedded-content,
    html[data-bs-theme="dark"] .lre-root.lre-embedded-content,
    body[data-bs-theme="dark"] .lre-root.lre-embedded-content,
    html[data-color-mode="dark"] .lre-root.lre-embedded-content,
    body[data-color-mode="dark"] .lre-root.lre-embedded-content,
    body.lre-dark .lre-root.lre-embedded-content,
    .lre-root.lre-embedded-content.statkiss-host-dark {
      --sk-embed-surface: #081120;
      --sk-embed-surface-soft: #0d1728;
      --sk-embed-toolbar: rgba(7,16,31,.94);
      --sk-embed-border: rgba(71,85,105,.54);
      --sk-embed-border-strong: rgba(96,165,250,.26);
      --sk-embed-text: #e5eefc;
      --sk-embed-text-soft: #cbd5e1;
      --sk-embed-muted: #94a3b8;
      --sk-embed-accent: #60a5fa;
      --sk-embed-accent-soft: rgba(96,165,250,.18);
      --sk-embed-hover: rgba(148,163,184,.08);
      --sk-embed-overlay: rgba(2,6,23,.42);
      --sk-embed-shadow: 0 24px 56px rgba(2,6,23,.34);
      color: var(--sk-embed-text) !important;
    }
    ...
    .lre-root.lre-embedded-content .lre-panel,
    .lre-root.lre-embedded-content .lre-searchbar {
      background: var(--sk-embed-toolbar) !important;
      border-color: var(--sk-embed-border) !important;
      color: var(--sk-embed-text) !important;
      box-shadow: none !important;
    }

    .lre-root.lre-embedded-content .lre-editor-page {
      background: var(--sk-embed-surface) !important;
      border: 1px solid var(--sk-embed-border) !important;
      color: var(--sk-embed-text) !important;
    }

    .lre-root.lre-embedded-content .lre-editor-page:empty::before,
    .lre-root.lre-embedded-content .lre-note,
    .lre-root.lre-embedded-content .lre-status-items,
    .lre-root.lre-embedded-content .lre-search-count,
    .lre-root.lre-embedded-content .lre-image-node figcaption,
    .lre-root.lre-embedded-content .lre-field > span,
    .lre-root.lre-embedded-content .lre-field label,
    .lre-root.lre-embedded-content .statkiss-inline-fields label {
      color: var(--sk-embed-muted) !important;
    }

    .lre-root.lre-embedded-content .lre-group {
      background: var(--sk-embed-surface-soft) !important;
      border-color: var(--sk-embed-border) !important;
    }
    ...
    .lre-root.lre-embedded-content .statkiss-mode-switcher button,
    .lre-root.lre-embedded-content .statkiss-code-toggle,
    .lre-root.lre-embedded-content .table-picker-actions button,
    .lre-root.lre-embedded-content .statkiss-inline-actions button {
      color: var(--sk-embed-text) !important;
      border-color: var(--sk-embed-border) !important;
    }

    .lre-root.lre-embedded-content .lre-btn:hover,
    .lre-root.lre-embedded-content .statkiss-code-toggle:hover,
    .lre-root.lre-embedded-content .table-picker-actions button:hover,
    .lre-root.lre-embedded-content .statkiss-inline-actions button:hover {
      background: var(--sk-embed-hover) !important;
      border-color: var(--sk-embed-accent) !important;
      color: var(--sk-embed-accent) !important;
    }

    .lre-root.lre-embedded-content .lre-btn.is-active,
    .lre-root.lre-embedded-content .statkiss-mode-switcher button.is-active {
      background: var(--sk-embed-accent-soft) !important;
      border-color: var(--sk-embed-accent) !important;
      color: var(--sk-embed-accent) !important;
      box-shadow: none !important;
    }
    ...
    .lre-root.lre-embedded-content .lre-source-card header,
    .lre-root.lre-embedded-content .lre-source-card textarea,
    .lre-root.lre-embedded-content .lre-source-preview,
    .lre-root.lre-embedded-content .lre-searchbar input,
    .lre-root.lre-embedded-content .lre-field input,
    .lre-root.lre-embedded-content .lre-field select,
    .lre-root.lre-embedded-content .lre-field textarea,
    .lre-root.lre-embedded-content .lre-dialog textarea,
    .lre-root.lre-embedded-content .lre-color-popover input,
    .lre-root.lre-embedded-content .table-picker-manual input,
    .lre-root.lre-embedded-content .statkiss-inline-fields input {
      background: var(--sk-embed-surface-soft) !important;
      border-color: var(--sk-embed-border) !important;
      color: var(--sk-embed-text) !important;
    ...
    .lre-root.lre-embedded-content .lre-searchbar input::placeholder,
    .lre-root.lre-embedded-content .lre-field input::placeholder,
    .lre-root.lre-embedded-content .lre-field textarea::placeholder,
    .lre-root.lre-embedded-content .lre-dialog textarea::placeholder,
    .lre-root.lre-embedded-content .table-picker-manual input::placeholder,
    .lre-root.lre-embedded-content .statkiss-inline-fields input::placeholder {
      color: var(--sk-embed-muted) !important;
    }

    .lre-root.lre-embedded-content .lre-editor-page blockquote {
      background: var(--sk-embed-surface-soft) !important;
      border-left-color: var(--sk-embed-accent) !important;
      color: var(--sk-embed-text-soft) !important;
    }

    .lre-root.lre-embedded-content .lre-inline-code {
      background: var(--sk-embed-surface-soft) !important;
      color: var(--sk-embed-text) !important;
    }

    .lre-root.lre-embedded-content .lre-editor-page hr {
      border-top-color: var(--sk-embed-border) !important;
    }

    .lre-root.lre-embedded-content .lre-editor-page a,
    .lre-root.lre-embedded-content .lre-source-preview a {
      color: var(--sk-embed-accent) !important;
    }

    .lre-root.lre-embedded-content .lre-editor-page table,
    .lre-root.lre-embedded-content .lre-editor-page th,
    .lre-root.lre-embedded-content .lre-editor-page td {
      border-color: var(--sk-embed-border) !important;
      color: var(--sk-embed-text) !important;
    }
    ...
    .lre-root.lre-embedded-content .lre-dialog-card,
    .lre-root.lre-embedded-content .lre-color-popover.is-open,
    .lre-root.lre-embedded-content .table-picker-pop,
    .lre-root.lre-embedded-content .statkiss-inline-card {
      background: var(--sk-embed-toolbar) !important;
      border-color: var(--sk-embed-border) !important;
      color: var(--sk-embed-text) !important;
      box-shadow: var(--sk-embed-shadow) !important;
    }

    .lre-root.lre-embedded-content .lre-color-popover::before,
    .lre-root.lre-embedded-content dialog.lre-dialog::backdrop {
      background: var(--sk-embed-overlay) !important;
    }

    .lre-root.lre-embedded-content .table-picker-cell,
    .lre-root.lre-embedded-content .statkiss-inline-grid button {
      background: var(--sk-embed-surface) !important;
      border-color: var(--sk-embed-border) !important;
    }

    .lre-root.lre-embedded-content .table-picker-cell.is-hot,
    .lre-root.lre-embedded-content .statkiss-inline-grid button.is-active {
      background: var(--sk-embed-accent-soft) !important;
      border-color: var(--sk-embed-accent) !important;
    }

    .lre-root.lre-embedded-content .table-picker-grid,
    .lre-root.lre-embedded-content .statkiss-inline-grid {
      background: var(--sk-embed-surface-soft) !important;
      border-color: var(--sk-embed-border) !important;
    }

    .lre-root.lre-embedded-content .table-picker-meta,
    .lre-root.lre-embedded-content .statkiss-inline-card h3,
    .lre-root.lre-embedded-content .lre-panel h2,
    .lre-root.lre-embedded-content .lre-panel h3 {
      color: var(--sk-embed-text) !important;
    }

    .lre-root.lre-embedded-content .lre-code-node,
    body.lre-dark .lre-root.lre-embedded-content .lre-code-node,
    .lre-root.lre-embedded-content.statkiss-host-dark .lre-code-node {
      background: var(--sk-embed-surface-soft) !important;
      border-color: var(--sk-embed-border) !important;
      color: var(--sk-embed-text) !important;
    }
    ...
    .lre-root.lre-embedded-content .lre-code-head,
    body.lre-dark .lre-root.lre-embedded-content .lre-code-head,
    .lre-root.lre-embedded-content.statkiss-host-dark .lre-code-head {
      background: rgba(255,255,255,.88) !important;
      border-color: var(--sk-embed-border) !important;
    }

    html.dark .lre-root.lre-embedded-content .lre-code-head,
    body.dark .lre-root.lre-embedded-content .lre-code-head,
    html[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-head,
    body[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-head,
    body.lre-dark .lre-root.lre-embedded-content .lre-code-head,
    .lre-root.lre-embedded-content.statkiss-host-dark .lre-code-head {
    ...
    html.dark .lre-root.lre-embedded-content .lre-code-head .lre-chip,
    body.dark .lre-root.lre-embedded-content .lre-code-head .lre-chip,
    html[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-head .lre-chip,
    body[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-head .lre-chip,
    body.lre-dark .lre-root.lre-embedded-content .lre-code-head .lre-chip,
    .lre-root.lre-embedded-content.statkiss-host-dark .lre-code-head .lre-chip,
    html.dark .lre-root.lre-embedded-content .statkiss-code-toggle,
    body.dark .lre-root.lre-embedded-content .statkiss-code-toggle,
    html[data-theme="dark"] .lre-root.lre-embedded-content .statkiss-code-toggle,
    body[data-theme="dark"] .lre-root.lre-embedded-content .statkiss-code-toggle,
    body.lre-dark .lre-root.lre-embedded-content .statkiss-code-toggle,
    .lre-root.lre-embedded-content.statkiss-host-dark .statkiss-code-toggle {
      background: rgba(15,23,42,.8) !important;
      border-color: rgba(96,165,250,.22) !important;
      color: #bfdbfe !important;
    }

    .lre-root.lre-embedded-content .lre-code-node .tok-comment { color: #64748b !important; }
    .lre-root.lre-embedded-content .lre-code-node .tok-key,
    .lre-root.lre-embedded-content .lre-code-node .tok-tag { color: #0f766e !important; }
    .lre-root.lre-embedded-content .lre-code-node .tok-string,
    .lre-root.lre-embedded-content .lre-code-node .tok-link { color: #b45309 !important; }
    .lre-root.lre-embedded-content .lre-code-node .tok-number { color: #7c3aed !important; }
    .lre-root.lre-embedded-content .lre-code-node .tok-attr { color: #1d4ed8 !important; }
    .lre-root.lre-embedded-content .lre-code-node .tok-fn,
    .lre-root.lre-embedded-content .lre-code-node .tok-title { color: #be123c !important; }
    .lre-root.lre-embedded-content .lre-code-node .tok-type,
    .lre-root.lre-embedded-content .lre-code-node .tok-bullet { color: #4f46e5 !important; }

    html.dark .lre-root.lre-embedded-content .lre-code-node .tok-comment,
    body.dark .lre-root.lre-embedded-content .lre-code-node .tok-comment,
    html[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-comment,
    body[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-comment,
    body.lre-dark .lre-root.lre-embedded-content .lre-code-node .tok-comment,
    .lre-root.lre-embedded-content.statkiss-host-dark .lre-code-node .tok-comment { color: #94a3b8 !important; }
    html.dark .lre-root.lre-embedded-content .lre-code-node .tok-key,
    body.dark .lre-root.lre-embedded-content .lre-code-node .tok-key,
    html.dark .lre-root.lre-embedded-content .lre-code-node .tok-tag,
    body.dark .lre-root.lre-embedded-content .lre-code-node .tok-tag,
    html[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-key,
    body[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-key,
    html[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-tag,
    body[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-tag,
    body.lre-dark .lre-root.lre-embedded-content .lre-code-node .tok-key,
    body.lre-dark .lre-root.lre-embedded-content .lre-code-node .tok-tag,
    .lre-root.lre-embedded-content.statkiss-host-dark .lre-code-node .tok-key,
    .lre-root.lre-embedded-content.statkiss-host-dark .lre-code-node .tok-tag { color: #5eead4 !important; }
    html.dark .lre-root.lre-embedded-content .lre-code-node .tok-string,
    body.dark .lre-root.lre-embedded-content .lre-code-node .tok-string,
    html.dark .lre-root.lre-embedded-content .lre-code-node .tok-link,
    body.dark .lre-root.lre-embedded-content .lre-code-node .tok-link,
    html[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-string,
    body[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-string,
    html[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-link,
    body[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-link,
    body.lre-dark .lre-root.lre-embedded-content .lre-code-node .tok-string,
    body.lre-dark .lre-root.lre-embedded-content .lre-code-node .tok-link,
    .lre-root.lre-embedded-content.statkiss-host-dark .lre-code-node .tok-string,
    .lre-root.lre-embedded-content.statkiss-host-dark .lre-code-node .tok-link { color: #fbbf24 !important; }
    html.dark .lre-root.lre-embedded-content .lre-code-node .tok-number,
    body.dark .lre-root.lre-embedded-content .lre-code-node .tok-number,
    html[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-number,
    body[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-number,
    body.lre-dark .lre-root.lre-embedded-content .lre-code-node .tok-number,
    .lre-root.lre-embedded-content.statkiss-host-dark .lre-code-node .tok-number { color: #c4b5fd !important; }
    html.dark .lre-root.lre-embedded-content .lre-code-node .tok-attr,
    body.dark .lre-root.lre-embedded-content .lre-code-node .tok-attr,
    html[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-attr,
    body[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-attr,
    body.lre-dark .lre-root.lre-embedded-content .lre-code-node .tok-attr,
    .lre-root.lre-embedded-content.statkiss-host-dark .lre-code-node .tok-attr { color: #93c5fd !important; }
    html.dark .lre-root.lre-embedded-content .lre-code-node .tok-fn,
    body.dark .lre-root.lre-embedded-content .lre-code-node .tok-fn,
    html.dark .lre-root.lre-embedded-content .lre-code-node .tok-title,
    body.dark .lre-root.lre-embedded-content .lre-code-node .tok-title,
    html[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-fn,
    body[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-fn,
    html[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-title,
    body[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-title,
    body.lre-dark .lre-root.lre-embedded-content .lre-code-node .tok-fn,
    body.lre-dark .lre-root.lre-embedded-content .lre-code-node .tok-title,
    .lre-root.lre-embedded-content.statkiss-host-dark .lre-code-node .tok-fn,
    .lre-root.lre-embedded-content.statkiss-host-dark .lre-code-node .tok-title { color: #fda4af !important; }
    html.dark .lre-root.lre-embedded-content .lre-code-node .tok-type,
    body.dark .lre-root.lre-embedded-content .lre-code-node .tok-type,
    html.dark .lre-root.lre-embedded-content .lre-code-node .tok-bullet,
    body.dark .lre-root.lre-embedded-content .lre-code-node .tok-bullet,
    html[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-type,
    body[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-type,
    html[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-bullet,
    body[data-theme="dark"] .lre-root.lre-embedded-content .lre-code-node .tok-bullet,
    body.lre-dark .lre-root.lre-embedded-content .lre-code-node .tok-type,
    body.lre-dark .lre-root.lre-embedded-content .lre-code-node .tok-bullet,
    .lre-root.lre-embedded-content.statkiss-host-dark .lre-code-node .tok-type,
    .lre-root.lre-embedded-content.statkiss-host-dark .lre-code-node .tok-bullet { color: #a5b4fc !important; }
    `;
  ...
  function getThemeFromClassList(el) {
  ...
  function getThemeFromAttributes(el) {
    if (!el || !el.getAttribute) return null;
    const keys = ['data-theme', 'theme', 'data-bs-theme', 'data-color-mode'];
    for (let i = 0; i < keys.length; i += 1) {
      const next = normalizeThemeValue(el.getAttribute(keys[i]));
      if (next) return next;
    }
    return null;
  }
  ...
  function getThemeFromStorage() {
    const keys = ['statkiss_theme', 'statkiss_appearance', 'appearance', 'theme', 'color-theme', 'mode'];
    for (let i = 0; i < keys.length; i += 1) {
      try {
        const next = normalizeThemeValue(localStorage.getItem(keys[i]));
        if (next) return next;
      } catch (error) {}
    }
    return null;
  ...
  function detectHostTheme() {
    if (document.body && document.body.classList && document.body.classList.contains('lre-dark')) return 'dark';
    const targets = [document.documentElement, document.body];
    for (let i = 0; i < targets.length; i += 1) {
      const themeByClass = getThemeFromClassList(targets[i]);
      if (themeByClass) return themeByClass;
      const themeByAttr = getThemeFromAttributes(targets[i]);
      if (themeByAttr) return themeByAttr;
    }
    const fromStorage = getThemeFromStorage();
    if (fromStorage) return fromStorage;
    return null;
  }

  function syncHostTheme(editor) {
    if (!editor || !editor.root) return;
    const nextTheme = detectHostTheme();
    const dark = nextTheme === 'dark';
    editor.root.classList.toggle('statkiss-host-dark', dark);
    editor.root.setAttribute('data-statkiss-host-theme', dark ? 'dark' : 'light');
  }

  function bindHostThemeSync(editor) {
    if (!editor || !editor.root) return;
    syncHostTheme(editor);
    if (editor.__statkissHostThemeSyncBound) return;
    editor.__statkissHostThemeSyncBound = true;

    const schedule = function(){ syncHostTheme(editor); };

    if (document.documentElement) {
      const observer = new MutationObserver(schedule);
      observer.observe(document.documentElement, { attributes: true, attributeFilter: WATCH_ATTRS });
      if (document.body) observer.observe(document.body, { attributes: true, attributeFilter: WATCH_ATTRS });
      editor.__statkissHostThemeObserver = observer;
    }

    window.addEventListener('statkiss:theme', schedule);
    ...
    if (window.matchMedia) {
      try {
        ...
        editor.__statkissHostThemeMedia = mq;
      } catch (error) {}
    }
  }

  function patchEditor(editor) {
    if (!editor || !editor.root || editor[PATCH_FLAG]) return editor;
    editor[PATCH_FLAG] = true;
    ensureStyle();
    bindHostThemeSync(editor);
    syncHostTheme(editor);
    try { console.info('[LocalRichEditor]', '20260403-dark-palette-patch applied'); } catch (error) {}
    return editor;
  }
  ...
  function waitForEditor(callback) {
    ...
    const timer = setInterval(function(){
      tries += 1;
      if (run() || tries > 320) clearInterval(timer);
    }, 50);
    window.addEventListener('local-rich-editor:ready', function(){
      if (run()) clearInterval(timer);
    }, { once: true });
  ...
  function wrapMount(name) {
    const original = window[name];
    if (typeof original !== 'function' || original.__statkissDarkPaletteWrapped) return;
    const wrapped = function() {
      const result = original.apply(this, arguments);
      if (result && typeof result.then === 'function') {
        return result.then(function(editor){ return patchEditor(editor); });
    ...
    wrapped.__statkissDarkPaletteWrapped = true;
    wrapped.__statkissDarkPaletteOriginal = original;
    window[name] = wrapped;
  }

  [
    'mountStatkissContentEditor',
    'mountEmbeddedContentEditor',
    'initStatkissContentEditor',
    'initEmbeddedContentEditor',
    'autoMountStatkissContentEditor',
    'mountContentEditor',
    'initContentEditor',
    'autoMountContentEditor',
    'mountLocalRichEditor',
    'createLocalRichEditor'
  ].forEach(wrapMount);
...
/* SolidEdit 0.0.4: flat formatting toolbar and compact document viewport. */
(function(){
  'use strict';
  if (typeof window === 'undefined' || window.__SOLID_EDIT_UI_0_0_4__) return;
  window.__SOLID_EDIT_UI_0_0_4__ = true;
  window.SOLID_EDIT_VERSION = '0.0.4';

  var STYLE_ID = 'solid-edit-ui-0-0-4';
  var WRAP_FLAG = '__solidEditUi004Wrapped';
  var S = '.lre-root.lre-ui-004[data-solid-edit-ui="0.0.4"]';
  ...
  function ensureStyle(){
    ...
    style.id = STYLE_ID;
    style.textContent = [
      S+'{color-scheme:light;--solid-edit-control:36px;--solid-edit-icon:20px;--solid-edit-label:12px;--solid-edit-pad:24px;--solid-edit-bar:#f8fafc;--solid-edit-paper:#fff;--solid-edit-line:#dce2e9;--solid-edit-ink:#243247;--solid-edit-muted:#64748b;--solid-edit-accent:#2563eb;--solid-edit-active:#e8f0fe;--solid-edit-hover:#edf1f5;--lre-radius:8px;--lre-btn-radius:6px;}',
      S+'[data-solid-edit-mode="mini"]{--solid-edit-control:28px;--solid-edit-icon:16px;--solid-edit-label:11px;--solid-edit-pad:12px;}',
      S+'.statkiss-host-dark,body.lre-dark '+S+',html.dark '+S+',[data-theme="dark"] '+S+'{color-scheme:dark;--solid-edit-bar:#17212f;--solid-edit-paper:#101923;--solid-edit-line:#344154;--solid-edit-ink:#e5edf7;--solid-edit-muted:#9caec4;--solid-edit-accent:#93b8ff;--solid-edit-active:#223c62;--solid-edit-hover:#253348;}',
      S+' .lre-topbar,'+S+' .lre-surface,'+S+' .lre-panel,'+S+' .lre-statusbar,'+S+' .lre-source-panel{border-radius:8px!important;box-shadow:none!important;}',
      S+' .lre-topbar{padding:8px!important;gap:8px!important;background:var(--solid-edit-bar)!important;}',
      S+' .lre-toolbar-scroll{min-width:0!important;width:100%!important;overflow:visible!important;padding:0!important;}',
      S+' .lre-toolbar,'+S+' #lreEmbedToolbarHost .lre-toolbar{display:flex!important;flex-wrap:wrap!important;align-items:center!important;gap:4px 6px!important;min-width:0!important;width:100%!important;padding:0!important;}',
      S+' .lre-toolbar .lre-group,'+S+' #lreEmbedToolbarHost .lre-group{display:inline-flex!important;align-items:center!important;flex-wrap:wrap!important;gap:2px!important;max-width:100%!important;margin:0!important;padding:0 6px 0 0!important;border:0!important;border-inline-end:1px solid var(--solid-edit-line)!important;border-radius:0!important;background:transparent!important;box-shadow:none!important;}',
      S+' .lre-toolbar .lre-group:last-child{border-inline-end:0!important;padding-inline-end:0!important;}',
      S+' .lre-toolbar .lre-btn,'+S+' #lreEmbedToolbarHost .lre-btn{display:inline-flex!important;align-items:center!important;justify-content:center!important;flex:0 0 auto!important;box-sizing:border-box!important;width:var(--solid-edit-control)!important;height:var(--solid-edit-control)!important;min-width:var(--solid-edit-control)!important;min-height:var(--solid-edit-control)!important;max-width:var(--solid-edit-control)!important;max-height:var(--solid-edit-control)!important;padding:0!important;margin:0!important;border:0!important;border-radius:6px!important;background:transparent!important;box-shadow:none!important;color:var(--solid-edit-ink)!important;}',
      S+' .lre-toolbar .lre-btn svg,'+S+' #lreEmbedToolbarHost .lre-btn svg{display:block!important;width:var(--solid-edit-icon)!important;height:var(--solid-edit-icon)!important;min-width:0!important;flex:0 0 auto!important;}',
      S+' .solid-edit-tool-label{display:block;font:650 var(--solid-edit-label)/1 ui-sans-serif,system-ui,sans-serif;letter-spacing:-.02em;color:inherit;white-space:nowrap;}',
      S+' .lre-toolbar .lre-btn:hover:not(:disabled),'+S+' #lreEmbedToolbarHost .lre-btn:hover:not(:disabled){background:var(--solid-edit-hover)!important;color:var(--solid-edit-ink)!important;}',
      S+' .lre-toolbar .lre-btn.is-active,'+S+' #lreEmbedToolbarHost .lre-btn.is-active,'+S+' .lre-toolbar .lre-btn[aria-pressed="true"]{background:var(--solid-edit-active)!important;color:var(--solid-edit-accent)!important;}',
      S+' .lre-toolbar .lre-btn:focus-visible,'+S+' #lreEmbedToolbarHost .lre-btn:focus-visible{outline:2px solid var(--solid-edit-accent)!important;outline-offset:1px!important;}',
      S+' .lre-toolbar .lre-btn:disabled{opacity:.4!important;cursor:default!important;}',
      S+' .lre-ribbon-toggle-group,'+S+' .statkiss-toolbar-size-group,'+S+' .lre-toolbar .lre-group[data-embed-empty="true"],'+S+' #lreEmbedToolbarHost .lre-group[data-embed-empty="true"],'+S+' .lre-toolbar .lre-btn[data-embed-hidden="true"],'+S+' #lreEmbedToolbarHost .lre-btn[data-embed-hidden="true"]{display:none!important;}',
      S+'.lre-embedded-content{padding:0!important;min-height:0!important;}',
      S+'.lre-embedded-content .lre-app{gap:8px!important;}',
      S+'.lre-embedded-content .lre-surface{padding:0!important;border:1px solid var(--solid-edit-line)!important;background:var(--solid-edit-paper)!important;overflow:hidden!important;}',
      S+'.lre-embedded-content #lreEmbedToolbarHost{position:relative!important;top:auto!important;display:flex!important;flex-wrap:wrap!important;gap:4px!important;padding:6px!important;border:0!important;border-bottom:1px solid var(--solid-edit-line)!important;border-radius:0!important;background:var(--solid-edit-bar)!important;box-shadow:none!important;}',
      S+'.lre-embedded-content .lre-editor-shell{gap:0!important;}',
      S+' .lre-editor-page{box-sizing:border-box!important;min-height:var(--solid-edit-content-min,320px)!important;max-height:var(--solid-edit-content-max,none)!important;overflow-y:auto!important;overscroll-behavior:contain;scrollbar-gutter:stable;scrollbar-color:var(--solid-edit-muted) var(--solid-edit-paper);width:100%!important;max-width:100%!important;margin:0!important;padding:var(--solid-edit-pad)!important;border:0!important;border-radius:0!important;background:var(--solid-edit-paper)!important;color:var(--solid-edit-ink)!important;box-shadow:none!important;font-size:16px;line-height:1.7;}',
      S+' .lre-editor-page>:first-child{margin-top:0!important;}',
      S+' .lre-editor-page:focus-visible{outline:2px solid var(--solid-edit-accent)!important;outline-offset:-2px!important;}',
      S+' .lre-code-node,'+S+' .lre-image-node,'+S+' .lre-math-node.block{border-radius:8px!important;}',
      S+' .lre-chip{border-radius:4px!important;min-height:24px!important;padding:0 6px!important;font-size:12px!important;}',
      S+' .lre-chip:empty{display:none!important;}',
      S+' .lre-panel input,'+S+' .lre-panel select,'+S+' .lre-panel textarea{box-sizing:border-box!important;min-width:0!important;max-width:100%!important;}',
      S+' .lre-panel .lre-field,'+S+' .lre-panel .statkiss-inline-fields,'+S+' .lre-panel .statkiss-inline-fields>*{min-width:0!important;max-width:100%!important;}',
      S+' .lre-statusbar{padding:6px 10px!important;font-size:12px!important;}',
      '@media(max-width:720px){'+S+'{--solid-edit-pad:16px;}'+S+'[data-solid-edit-mode="mini"]{--solid-edit-pad:12px;}'+S+' .lre-toolbar,'+S+' #lreEmbedToolbarHost .lre-toolbar{gap:4px!important;}'+S+' .lre-toolbar .lre-group,'+S+' #lreEmbedToolbarHost .lre-group{gap:2px!important;padding-inline-end:4px!important;}}'
    ].join('');
    document.head.appendChild(style);
  }

  function modeFor(options, root){
    if (options && Object.prototype.hasOwnProperty.call(options, 'toolbarSize')) return options.toolbarSize === 'mini' ? 'mini' : 'full';
    return root.getAttribute('data-solid-edit-mode') === 'mini' ? 'mini' : 'full';
  }
  function pixels(value, fallback){
    return typeof value === 'number' && Number.isFinite(value) ? Math.max(80, Math.min(2400, Math.round(value))) : fallback;
  }
  function normalizeToolbar(editor){
    var root = editor.root;
    var toolbar = root.querySelector('.lre-toolbar');
    if (!toolbar) return;
    root.querySelectorAll('[data-ribbon-toggle], .lre-ribbon-toggle-group, .statkiss-toolbar-size-group').forEach(function(node){
      var group = node.closest('.lre-ribbon-toggle-group') || node;
      group.remove();
    });
    toolbar.setAttribute('role', 'toolbar');
    toolbar.setAttribute('aria-label', String(root.getAttribute('lang') || '').indexOf('ko') === 0 ? '문서 서식' : 'Document formatting');
    var host = root.querySelector('#lreEmbedToolbarHost');
    if (host) {
      host.removeAttribute('role');
      host.removeAttribute('aria-label');
      Array.from(host.children).forEach(function(child){
        if (child.classList.contains('lre-group')) toolbar.appendChild(child);
      });
    }
    toolbar.querySelectorAll('.lre-group').forEach(function(group){
      if (group.dataset.embedEmpty !== 'true') group.hidden = false;
      group.setAttribute('role', 'group');
    });
    toolbar.querySelectorAll('.lre-btn svg').forEach(function(svg){
      var label = svg.querySelector('text');
      if (!label) return;
      var span = document.createElement('span');
      span.className = 'solid-edit-tool-label';
      span.setAttribute('aria-hidden', 'true');
      span.textContent = label.textContent;
      svg.replaceWith(span);
    });
  }
  function applyMode(editor, options){
    if (!editor || !editor.root) return editor;
    ensureStyle();
    var root = editor.root;
    var mode = modeFor(options, root);
    var minHeight = pixels(options && options.minHeight, mode === 'mini' ? 160 : 320);
    var maxHeight = pixels(options && options.maxHeight, mode === 'mini' ? 320 : null);
    root.classList.add('lre-ui-004', 'lre-tools-always-visible', 'lre-ribbon-expanded');
    root.setAttribute('data-solid-edit-ui', '0.0.4');
    root.setAttribute('data-solid-edit-mode', mode);
    root.style.setProperty('--solid-edit-content-min', minHeight + 'px');
    root.style.setProperty('--solid-edit-content-max', maxHeight === null ? 'none' : Math.max(minHeight, maxHeight) + 'px');
    editor.state = editor.state || {};
    editor.state.ribbonExpanded = true;
    normalizeToolbar(editor);
    if (!editor.__solidEditUiObserver && typeof MutationObserver === 'function') {
      editor.__solidEditUiObserver = new MutationObserver(function(){ normalizeToolbar(editor); });
      var toolbar = root.querySelector('.lre-toolbar');
      if (toolbar) editor.__solidEditUiObserver.observe(toolbar, { childList:true, subtree:true });
    }
    return editor;
  }
  function optionsFor(name, args){
    return /^(mountLocalRichEditor|createLocalRichEditor)$/.test(name) ? args[0] || {} :
      /^(mountStatkissContentEditor|mountEmbeddedContentEditor|mountContentEditor)$/.test(name) ? args[1] || {} : args[0] || {};
  }
  function wrap(name){
    var original = window[name];
    if (!isFn(original) || original[WRAP_FLAG]) return;
    var wrapped = function(){
      var options = optionsFor(name, arguments);
      var result = original.apply(this, arguments);
      return result && isFn(result.then) ? result.then(function(editor){ return applyMode(editor, options); }) : applyMode(result, options);
    };
    wrapped[WRAP_FLAG] = true;
    window[name] = wrapped;
  }
  ['mountLocalRichEditor','createLocalRichEditor','mountStatkissContentEditor','mountEmbeddedContentEditor','mountContentEditor','autoMountStatkissContentEditor','autoMountContentEditor','initStatkissContentEditor','initEmbeddedContentEditor','initContentEditor'].forEach(wrap);
  var destroy = window.destroyLocalRichEditor;
  if (isFn(destroy)) window.destroyLocalRichEditor = function(editor){
    var target = editor || window.localRichEditor || window.__localRichEditor;
    if (target && target.__solidEditUiObserver) {
      target.__solidEditUiObserver.disconnect();
      delete target.__solidEditUiObserver;
    }
    return destroy.apply(this, arguments);
  };
  window.addEventListener('local-rich-editor:ready', function(event){
    applyMode(event && event.detail && event.detail.editor, window.CONTENT_EDITOR_CONFIG || {});
  });
  applyMode(window.localRichEditor || window.__localRichEditor, window.CONTENT_EDITOR_CONFIG || {});
})();
